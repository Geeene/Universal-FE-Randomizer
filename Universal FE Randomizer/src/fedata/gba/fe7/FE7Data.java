...
public class FE7Data implements GBAFECharacterProvider, GBAFEClassProvider, GBAFEItemProvider {
	...
	// This is more than just promotion items, but they're all clustered together
	// around here.
	// I actually suspect this is the table for the class restrictions for item
	// usage.
	...
	// We can hack the check for mode select by embedding a header that is
	// sufficient to enable mode select from the beginning without real clear data
	// by hijacking the part of the ASM that checks the SRAM header. It's apparently
	// sufficient for the first 0x64 bytes of SRAM to figure out
	// whether there exists clear data, so when the game tries to read the SRAM
	// header, we modify it to read from ROM instead of SRAM and
	// bake in a known good "cleared" header into the ROM. We can then repoint it to
	// read from our own pointer to the hardcoded header instead of
	// the usual SRAM offset (normally mapped to 0xE000000).
	public static final long HardcodedSRAMHeaderOffset = 0x9E554L;
	public static final long DefaultSRAMHeaderPointer = 0xCE3B58L; // This is what it normally is (the value at this
																	// address is 0xE000000, which is how SRAM is
																	// mapped).

	// The mode select hard codes the portraits and classes that show up.
	// We should figure out who our lords are (if we randomize recruitment) and
	// replace those faces accordingly.
	// The faces are stored in 12 bytes, 4 bytes each, starting with Lyn, then
	// Eliwood, then Hector
	// The default data was 16 00 00 00 02 00 00 00 0C 00 00 00 with 16 being Lyn's
	// FaceID, 02 being Eliwood's FaceID, and 0C being Hector's FaceID.
	public static final long ModeSelectPortraitOffset = 0x418DB4L;
	// Sprite played is actually right before it, formatted int he same way, just
	// with animation IDs.
	// The default data was 0E 00 00 00 00 00 00 00 06 00 00 00 with 0E being Lyn
	// Lord's battle animation, 00 being Eliwood Lord's Battle Animation, and 06
	// being Hector Lord's battle animation.
	public static final long ModeSelectClassAnimationOffset = 0x418DA8L;
	// The weapon type is stored elsewhere. Stored in triplets of text indices. Each
	// entry is 12 bytes long and contains their name, their weapon type, and a
	// short description (not sure where that's used)
	// The default data is:
	// DE 04 00 00 - 'Lyn'
	...
	// B9 12 00 00 - 'Axe'
	// Obviously, there's some sharing for Lyn and Eliwood's weapon, so we'll jack
	// an unused string's text entry. This is the only one we'll need to change.
	...
	// I'm not sure if this is used, but we're going to steal it for Eliwood's
	// slot's weapon.
	...
	// The item ID bytes can be replaced at 0x7D0C2 (which is normally 0x3E for
	// Lightning)
	...
	private static final List<AddressRange> createFreeRangeList() {
		...
		// There's stuff here, but hacking docs seem to think these are free to use
		// so...
	...
	public enum CharacterAndClassAbility1Mask {
		USE_MOUNTED_AID(0x1), CANTO(0x2), STEAL(0x4), USE_LOCKPICKS(0x8), DANCE(0x10), PLAY(0x20), CRITICAL_15(0x40),
		BALLISTA(0x80);
	...
	public enum CharacterAndClassAbility3Mask {
		UNUSED_WEAPON_LOCK(0x1), WO_DAO_LOCK(0x2), DRAGONSTONE_LOCK(0x4), MORPHS_VAIDA(0x8), UNCONTROLLABLE(0x10),
		TRIANGLE_ATTACK(0x20), UNUSED_TRIANGLE_ATTACK(0x40), UNKNOWN(0x80);
	...
	public enum CharacterAndClassAbility4Mask {
		NO_EXPERIENCE_GIVEN(0x1), LETHALITY(0x2), MAGIC_SEAL(0x4), DROP_LAST_ITEM(0x8), ELIWOOD_LOCK(0x10),
		HECTOR_LOCK(0x20), LYN_LOCK(0x40), ATHOS_LOCK(0x80);
	...
	public enum Character implements GBAFECharacter {
		...
		ELIWOOD(0x01), HECTOR(0x02), RAVEN(0x04), GEITZ(0x05), GUY(0x06), KAREL(0x07), DORCAS(0x08), BARTRE(0x09),
		OSWIN(0x0B), REBECCA(0x0E), LOUISE(0x0F), LUCIUS(0x10), SERRA(0x11), RENAULT(0x12), ERK(0x13), NINO(0x14),
		PENT(0x15), CANAS(0x16), LOWEN(0x19), MARCUS(0x1A), PRISCILLA(0x1B), FIORA(0x1E), FARINA(0x1F), HEATH(0x20),
		VAIDA(0x21), HAWKEYE(0x22), MATTHEW(0x23), JAFFAR(0x24), NINIAN(0x25), NILS(0x26), ATHOS(0x27), WALLACE(0x2C),
		LYN(0x2D), WIL(0x2E), KENT(0x2F), SAIN(0x30), FLORINA(0x31), RATH(0x32), DART(0x33), ISADORA(0x34),
		LEGAULT(0x36), KARLA(0x37), HARKEN(0x38),

		FARGUS(0x0C), MERLINUS(0x28), UTHER(0x2A), VAIDA_BOSS(0x2B), ELENORA(0x35), LEILA(0x39), BRAMIMOND(0x3A),
		ZEPHIEL(0x7A), ELBERT(0x7B), BRENDAN(0x84), NATALIE(0x9E), TACTICIAN(0xCD),

		LYN_TUTORIAL(0x03), WIL_TUTORIAL(0x0D), KENT_TUTORIAL(0x17), SAIN_TUTORIAL(0x18), RATH_TUTORIAL(0x1C),
		FLORINA_TUTORIAL(0x1D),

		NILS_FINALCHAPTER(0x29),

		GROZNYI(0x3C), WIRE(0x3D), ZAGAN(0x3F), BOIES(0x40), PUZON(0x41), SANTALS(0x43), ERIK(0x45), SEALEN(0x46),
		BAUKER(0x47), BERNARD(0x48), DAMIAN(0x49), ZOLDAM(0x4A), UHAI(0x4B), AION(0x4C), DARIN(0x4D), CAMERON(0x4E),
		OLEG(0x4F), EUBANS(0x50), URSULA(0x51), PAUL(0x53), JASMINE(0x54), PASCAL(0x57), KENNETH(0x58), JERME(0x59),
		MAXIME(0x5A), SONIA(0x5B), TEODOR(0x5C), GEORG(0x5D), KAIM(0x5E), DENNING(0x60), LIMSTELLA(0x85), BATTA(0x87),
		ZUGU(0x89), GLASS(0x8D), MIGAL(0x8E), CARJIGA(0x94), BUG(0x99), BOOL(0x9F), HEINTZ(0xA6), BEYARD(0xAD),
		YOGI(0xB6), EAGLER(0xBE), LUNDGREN(0xC5),

		LLOYD_FFO(0x63), LINUS_FFO(0x64), LLOYD_COD(0x65), LINUS_COD(0x66),

		JERME_MORPH(0x56), LLOYD_MORPH(0xF4), LINUS_MORPH(0xF5), BRENDAN_MORPH(0xF6), UHAI_MORPH(0xF7),
		URSULA_MORPH(0xF8), KENNETH_MORPH(0xF9), DARIN_MORPH(0xFA),
		...
		public static Set<Character> allPlayableCharacters = new HashSet<Character>(Arrays.asList(ELIWOOD, HECTOR,
				RAVEN, GEITZ, GUY, KAREL, DORCAS, BARTRE, OSWIN, REBECCA, LOUISE, LUCIUS, SERRA, RENAULT, ERK, NINO,
				PENT, CANAS, LOWEN, MARCUS, PRISCILLA, FIORA, FARINA, HEATH, VAIDA, HAWKEYE, MATTHEW, JAFFAR, NINIAN,
				NILS, WALLACE, LYN, WIL, KENT, SAIN, FLORINA, RATH, DART, ISADORA, LEGAULT, KARLA, HARKEN, LYN_TUTORIAL,
				WIL_TUTORIAL, KENT_TUTORIAL, SAIN_TUTORIAL, RATH_TUTORIAL, FLORINA_TUTORIAL, NILS_FINALCHAPTER));

		public static Set<Character> allBossCharacters = new HashSet<Character>(Arrays.asList(GROZNYI, WIRE, ZAGAN,
				BOIES, PUZON, ERIK, SEALEN, BAUKER, BERNARD, DAMIAN, ZOLDAM, UHAI, AION, DARIN, CAMERON, OLEG, EUBANS,
				URSULA, PAUL, JASMINE, PASCAL, KENNETH, JERME, MAXIME, BRENDAN, SONIA, TEODOR, GEORG, KAIM, DENNING,
				LIMSTELLA, BATTA, ZUGU, GLASS, MIGAL, CARJIGA, BUG, BOOL, HEINTZ, BEYARD, YOGI, EAGLER, LUNDGREN,
				LLOYD_FFO, LINUS_FFO, LLOYD_COD, LINUS_COD, JERME_MORPH, LLOYD_MORPH, LINUS_MORPH, BRENDAN_MORPH,
				UHAI_MORPH, URSULA_MORPH, KENNETH_MORPH, DARIN_MORPH));
		public static Set<Character> restrictedClassCharacters = new HashSet<Character>(
				Arrays.asList(FIORA, FARINA, VAIDA));

		public static Set<Character> allLords = new HashSet<Character>(
				Arrays.asList(ELIWOOD, HECTOR, LYN, LYN_TUTORIAL));
		public static Set<Character> allThieves = new HashSet<Character>(Arrays.asList(MATTHEW, LEGAULT, JAFFAR));
		public static Set<Character> doNotChange = new HashSet<Character>(Arrays.asList(NERGAL, DRAGON, KISHUNA, FARGUS,
				MERLINUS, UTHER, ELENORA, LEILA, BRAMIMOND, ZEPHIEL, ELBERT, NATALIE, TACTICIAN));

		public static Set<Character> charactersThatRequireRange = new HashSet<Character>(
				Arrays.asList(ERK, RATH, RATH_TUTORIAL));
		...
		public static Set<Character> requiredAttackers = new HashSet<Character>(
				Arrays.asList(LYN, LYN_TUTORIAL, ELIWOOD, HECTOR, JAFFAR, RATH, ERK, RATH_TUTORIAL));
		public static Set<Character> femaleSet = new HashSet<Character>(Arrays.asList(REBECCA, LOUISE, SERRA, NINO,
				PRISCILLA, FIORA, FARINA, VAIDA, FLORINA, ISADORA, KARLA, VAIDA_BOSS, ELENORA, LEILA, NATALIE,
				LYN_TUTORIAL, FLORINA_TUTORIAL, URSULA, SONIA, LIMSTELLA, URSULA_MORPH, LYN, NINIAN));
		...
		public static Set<Character> allSpecial = new HashSet<Character>(
				Arrays.asList(NILS, NINIAN, NILS_FINALCHAPTER));
		...
		private static Map<Character, Set<Integer>> createMultiPortraitMap() {
			...
			map.put(ELIWOOD,
					new HashSet<Integer>(Arrays.asList(0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B)));
			map.put(HECTOR,
					new HashSet<Integer>(Arrays.asList(0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15)));
	...
	public enum CharacterClass implements GBAFEClass {
		...
		MERCENARY(0x0A), MYRMIDON(0x0E), FIGHTER(0x12), KNIGHT(0x14), ARCHER(0x18), MONK(0x1C), MAGE(0x20),
		SHAMAN(0x24), CAVALIER(0x28), NOMAD(0x2E), WYVERNKNIGHT(0x34), SOLDIER(0x38), BRIGAND(0x39), PIRATE(0x3A),
		THIEF(0x3C), BARD(0x41), CORSAIR(0x50),
		...
		HERO(0x0C), SWORDMASTER(0x10), WARRIOR(0x13), GENERAL(0x16), SNIPER(0x1A), BISHOP(0x1E), SAGE(0x22),
		DRUID(0x26), PALADIN(0x2A), NOMADTROOPER(0x30), WYVERNLORD(0x36), BERSERKER(0x3B), ASSASSIN(0x3E),

		SWORDMASTER_F(0x11), SNIPER_F(0x1B), BISHOP_F(0x1F), SAGE_F(0x23), PALADIN_F(0x2B), VALKYRIE(0x2D),
		FALCONKNIGHT(0x33), WYVERNLORD_F(0x37),
		...
		public static Set<CharacterClass> allMaleClasses = new HashSet<CharacterClass>(
				Arrays.asList(LORD_ELIWOOD, LORD_HECTOR, MERCENARY, MYRMIDON, FIGHTER, KNIGHT, ARCHER, MONK, MAGE,
						SHAMAN, CAVALIER, NOMAD, WYVERNKNIGHT, SOLDIER, BRIGAND, PIRATE, THIEF, BARD, CORSAIR, HERO,
						SWORDMASTER, WARRIOR, GENERAL, SNIPER, BISHOP, SAGE, DRUID, PALADIN, NOMADTROOPER, WYVERNLORD,
						BERSERKER, ASSASSIN, LORD_KNIGHT, GREAT_LORD));
		public static Set<CharacterClass> allFemaleClasses = new HashSet<CharacterClass>(Arrays.asList(LORD_LYN,
				BLADE_LORD, ARCHER_F, CLERIC, MAGE_F, TROUBADOUR, PEGASUSKNIGHT, DANCER, SWORDMASTER_F, SNIPER_F,
		...
		public static Set<CharacterClass> allLordClasses = new HashSet<CharacterClass>(
				Arrays.asList(LORD_ELIWOOD, LORD_LYN, LORD_HECTOR, LORD_KNIGHT, BLADE_LORD, GREAT_LORD));
		...
		public static Set<CharacterClass> allUnpromotedClasses = new HashSet<CharacterClass>(
				Arrays.asList(LORD_ELIWOOD, LORD_LYN, LORD_HECTOR, MERCENARY, MYRMIDON, FIGHTER, KNIGHT, ARCHER, MONK,
						MAGE, SHAMAN, CAVALIER, NOMAD, WYVERNKNIGHT, SOLDIER, BRIGAND, PIRATE, THIEF, BARD, CORSAIR,
						ARCHER_F, CLERIC, MAGE_F, TROUBADOUR, PEGASUSKNIGHT, DANCER));
		public static Set<CharacterClass> allPromotedClasses = new HashSet<CharacterClass>(
				Arrays.asList(LORD_KNIGHT, BLADE_LORD, GREAT_LORD, HERO, SWORDMASTER, WARRIOR, GENERAL, SNIPER, BISHOP,
						SAGE, DRUID, PALADIN, NOMADTROOPER, WYVERNLORD, BERSERKER, ASSASSIN, SWORDMASTER_F, SNIPER_F,
						BISHOP_F, SAGE_F, PALADIN_F, VALKYRIE, FALCONKNIGHT, WYVERNLORD_F, UBER_SAGE));
		public static Set<CharacterClass> allPacifistClasses = new HashSet<CharacterClass>(
				Arrays.asList(DANCER, BARD, CLERIC, TROUBADOUR));
		public static Set<CharacterClass> allMeleeLockedClasses = new HashSet<CharacterClass>(
				Arrays.asList(MYRMIDON, MERCENARY, LORD_LYN, LORD_ELIWOOD, THIEF));

		public static Set<CharacterClass> allValidClasses = new HashSet<CharacterClass>(Arrays.asList(LORD_ELIWOOD,
				LORD_HECTOR, MERCENARY, MYRMIDON, FIGHTER, KNIGHT, ARCHER, MONK, MAGE, SHAMAN, CAVALIER, NOMAD,
				WYVERNKNIGHT, SOLDIER, BRIGAND, PIRATE, THIEF, BARD, CORSAIR, HERO, SWORDMASTER, WARRIOR, GENERAL,
				SNIPER, BISHOP, SAGE, DRUID, PALADIN, NOMADTROOPER, WYVERNLORD, BERSERKER, ASSASSIN, LORD_LYN,
				BLADE_LORD, ARCHER_F, CLERIC, MAGE_F, TROUBADOUR, PEGASUSKNIGHT, DANCER, SWORDMASTER_F, SNIPER_F,
				BISHOP_F, SAGE_F, PALADIN_F, VALKYRIE, FALCONKNIGHT, WYVERNLORD_F, LORD_KNIGHT, GREAT_LORD, UBER_SAGE));

		public static Set<CharacterClass> allPlayerOnlyClasses = new HashSet<CharacterClass>(
				Arrays.asList(DANCER, BARD));

		public static Set<CharacterClass> flyingClasses = new HashSet<CharacterClass>(
				Arrays.asList(WYVERNKNIGHT, WYVERNLORD, PEGASUSKNIGHT, FALCONKNIGHT));

		public static Set<CharacterClass> meleeOnlyClasses = new HashSet<CharacterClass>(Arrays.asList(LORD_ELIWOOD,
				MERCENARY, MYRMIDON, THIEF, SWORDMASTER, ASSASSIN, LORD_LYN, SWORDMASTER_F));
		public static Set<CharacterClass> rangedOnlyClasses = new HashSet<CharacterClass>(
				Arrays.asList(ARCHER, NOMAD, SNIPER, ARCHER_F, SNIPER_F));

		public static Set<CharacterClass> allMountedClasses = new HashSet<CharacterClass>(
		...
		public static Set<CharacterClass> targetClassesForRandomization(CharacterClass sourceClass, boolean isForEnemy,
				...
				Boolean requireAttack, Boolean requiresRange, Boolean applyRestrictions, Boolean restrictGender) {
	...
	public enum Item implements GBAFEItem {
		...
		SILVER_BLADE(0x07), POISON_SWORD(0x08), RAPIER(0x09), MANI_KATTI(0x0A), BRAVE_SWORD(0x0B), WO_DAO(0x0C),
		KILLING_EDGE(0x0D), ARMORSLAYER(0x0E), WYRMSLAYER(0x0F), LIGHT_BRAND(0x10), RUNE_SWORD(0x11), LANCEREAVER(0x12),
		LONGSWORD(0x13), EMBLEM_SWORD(0x80), DURANDAL(0x84), SOL_KATTI(0x8C), REGAL_BLADE(0x90), WIND_SWORD(0x99),

		IRON_LANCE(0x14), SLIM_LANCE(0x15), STEEL_LANCE(0x16), SILVER_LANCE(0x17), POISON_LANCE(0x18),
		BRAVE_LANCE(0x19), KILLER_LANCE(0x1A), HORSESLAYER(0x1B), JAVELIN(0x1C), SPEAR(0x1D), AXEREAVER(0x1E),
		EMBLEM_LANCE(0x81), REX_HASTA(0x91), HEAVY_SPEAR(0x94), SHORT_SPEAR(0x95),
		...
		SWORDSLAYER(0x2B), DRAGON_AXE(0x59), EMBLEM_AXE(0x82), ARMADS(0x85), WOLF_BEIL(0x8D), BASILIKOS(0x92),

		IRON_BOW(0x2C), STEEL_BOW(0x2D), SILVER_BOW(0x2E), POISON_BOW(0x2F), KILLER_BOW(0x30), BRAVE_BOW(0x31),
		SHORT_BOW(0x32), LONGBOW(0x33), EMBLEM_BOW(0x83), RIENFLECHE(0x93),
		...
		HEAL(0x4A), MEND(0x4B), RECOVER(0x4C), PHYSIC(0x4D), FORTIFY(0x4E), RESTORE(0x4F), SILENCE(0x50), SLEEP(0x51),
		BERSERK(0x52), WARP(0x53), RESCUE(0x54), TORCH_STAFF(0x55), HAMMERNE(0x56), UNLOCK(0x57), BARRIER(0x58),

		ANGELIC_ROBE(0x5A), ENERGY_RING(0x5B), SECRET_BOOK(0x5C), SPEEDWINGS(0x5D), GODDESS_ICON(0x5E),
		DRAGONSHIELD(0x5F), TALISMAN(0x60), BOOTS(0x61), BODY_RING(0x62),

		HERO_CREST(0x63), KNIGHT_CREST(0x64), ORION_BOLT(0x65), ELYSIAN_WHIP(0x66), GUIDING_RING(0x67),
		EARTH_SEAL(0x87), HEAVEN_SEAL(0x89), EMBLEM_SEAL(0x8A), FELL_CONTRACT(0x8B), OCEAN_SEAL(0x96), AFA_DROPS(0x88),

		CHEST_KEY(0x68), CHEST_KEY_5(0x78), DOOR_KEY(0x69), LOCKPICK(0x6A), VULNERARY(0x6B), ELIXIR(0x6C),
		PURE_WATER(0x6D), ANTITOXIN(0x6E), TORCH(0x6F), MINE(0x79), LIGHT_RUNE(0x7A),
		...
		public enum FE7WeaponRank {
			...
			private FE7WeaponRank(final int value) {
				this.value = value;
			}
		...
		public enum FE7WeaponType {
			...
			private FE7WeaponType(final int id) {
				ID = id;
			}
		...
		public enum Ability3Mask {
			NONE(0x00), NEGATE_DEFENSE(0x02), ELIWOOD_LOCK(0x04), HECTOR_LOCK(0x08), LYN_LOCK(0x10), ATHOS_LOCK(0x20);
		...
		public static Set<Item> allSwords = new HashSet<Item>(Arrays.asList(IRON_SWORD, SLIM_SWORD, STEEL_SWORD,
				SILVER_SWORD, IRON_BLADE, STEEL_BLADE, SILVER_BLADE, POISON_SWORD, RAPIER, MANI_KATTI, BRAVE_SWORD,
				WO_DAO, KILLING_EDGE, ARMORSLAYER, WYRMSLAYER, LIGHT_BRAND, RUNE_SWORD, LANCEREAVER, LONGSWORD,
				EMBLEM_SWORD, DURANDAL, SOL_KATTI, REGAL_BLADE, WIND_SWORD));
		public static Set<Item> allLances = new HashSet<Item>(Arrays.asList(IRON_LANCE, SLIM_LANCE, STEEL_LANCE,
				SILVER_LANCE, POISON_LANCE, BRAVE_LANCE, KILLER_LANCE, HORSESLAYER, JAVELIN, SPEAR, AXEREAVER,
		...
		public static Set<Item> allAxes = new HashSet<Item>(Arrays.asList(IRON_AXE, STEEL_AXE, SILVER_AXE, POISON_AXE,
				...
				DRAGON_AXE, EMBLEM_AXE, ARMADS, WOLF_BEIL, BASILIKOS));
		public static Set<Item> allBows = new HashSet<Item>(Arrays.asList(IRON_BOW, STEEL_BOW, SILVER_BOW, POISON_BOW,
				KILLER_BOW, BRAVE_BOW, SHORT_BOW, LONGBOW, EMBLEM_BOW, RIENFLECHE));
		public static Set<Item> allAnima = new HashSet<Item>(
				Arrays.asList(FIRE, THUNDER, ELFIRE, BOLTING, FIMBULVETR, FORBLAZE, EXCALIBUR));
		public static Set<Item> allLight = new HashSet<Item>(
				Arrays.asList(LIGHTNING, SHINE, DIVINE, PURGE, AURA, LUCE, AUREOLA));
		public static Set<Item> allDark = new HashSet<Item>(
				Arrays.asList(FLUX, LUNA, NOSFERATU, ECLIPSE, FENRIR, GESPENST));
		public static Set<Item> allHealingStaves = new HashSet<Item>(
				Arrays.asList(HEAL, MEND, RECOVER, PHYSIC, FORTIFY));
		...
		public static Set<Item> allStatBoosters = new HashSet<Item>(Arrays.asList(ANGELIC_ROBE, ENERGY_RING,
				SECRET_BOOK, SPEEDWINGS, GODDESS_ICON, DRAGONSHIELD, TALISMAN, BOOTS, BODY_RING, AFA_DROPS));
		public static Set<Item> allPromotionItems = new HashSet<Item>(
				Arrays.asList(HERO_CREST, KNIGHT_CREST, ORION_BOLT, ELYSIAN_WHIP, GUIDING_RING, EARTH_SEAL, HEAVEN_SEAL,
						EMBLEM_SEAL, FELL_CONTRACT, OCEAN_SEAL));
		public static Set<Item> allDancingRings = new HashSet<Item>(
				Arrays.asList(FILLA_MIGHT, NINI_GRACE, THOR_IRE, SET_LITANY));
		public static Set<Item> allSpecialItems = new HashSet<Item>(
				Arrays.asList(DELPHI_SHIELD, MEMBER_CARD, IRON_RUNE, SILVER_CARD));
		public static Set<Item> allMoneyItems = new HashSet<Item>(
				Arrays.asList(WHITE_GEM, BLUE_GEM, RED_GEM, GOLD_3000, GOLD_5000));
		...
		public static Set<Item> usableItems = new HashSet<Item>(Arrays.asList(CHEST_KEY, CHEST_KEY_5, DOOR_KEY,
				LOCKPICK, VULNERARY, ELIXIR, PURE_WATER, ANTITOXIN, TORCH, MINE, LIGHT_RUNE));

		public static Set<Item> allSiegeTomes = new HashSet<Item>(Arrays.asList(BOLTING, PURGE, ECLIPSE));

		public static Set<Item> allPotentialRewards = new HashSet<Item>(
				Arrays.asList(/* IRON_SWORD, SLIM_SWORD, EMBLEM_SWORD, IRON_LANCE, SLIM_LANCE, */ JAVELIN,
						/* EMBLEM_LANCE, POISON_LANCE, */ HAND_AXE, /* IRON_AXE, EMBLEM_AXE, */ STEEL_AXE, DEVIL_AXE,
						/* IRON_BOW, EMBLEM_BOW, FIRE, LIGHTNING, HEAL, POISON_SWORD, */ STEEL_SWORD, IRON_BLADE,
						ARMORSLAYER, LONGSWORD, WO_DAO, STEEL_LANCE, HEAVY_SPEAR, HORSESLAYER,
						/* POISON_AXE, */ HALBERD, HAMMER, /* POISON_BOW, */
						SHORT_BOW, LONGBOW, STEEL_BOW, THUNDER, SHINE, FLUX, MEND, TORCH_STAFF, UNLOCK, STEEL_BLADE,
						KILLING_EDGE, WYRMSLAYER, LIGHT_BRAND, LANCEREAVER, SHORT_SPEAR, KILLER_LANCE, AXEREAVER,
						DRAGON_AXE, KILLER_AXE, SWORDREAVER, SWORDSLAYER, KILLER_BOW, ELFIRE, DIVINE, NOSFERATU,
						RECOVER, RESTORE, HAMMERNE, BARRIER, BRAVE_SWORD, WIND_SWORD, BRAVE_LANCE, SPEAR, BRAVE_AXE,
						BRAVE_BOW, BOLTING, PURGE, ECLIPSE, PHYSIC, SILENCE, SLEEP, BERSERK, RESCUE, SILVER_SWORD,
						SILVER_BLADE, RUNE_SWORD, SILVER_LANCE, TOMAHAWK, SILVER_AXE, SILVER_BOW, FIMBULVETR, AURA,
						FENRIR, FORTIFY, WARP, ANGELIC_ROBE, ENERGY_RING, SECRET_BOOK, SPEEDWINGS, GODDESS_ICON,
						DRAGONSHIELD, TALISMAN, BOOTS, BODY_RING, AFA_DROPS, HERO_CREST, KNIGHT_CREST, ORION_BOLT,
						ELYSIAN_WHIP, GUIDING_RING, EARTH_SEAL, HEAVEN_SEAL, EMBLEM_SEAL, FELL_CONTRACT, OCEAN_SEAL,
						FILLA_MIGHT, NINI_GRACE, THOR_IRE, SET_LITANY, DELPHI_SHIELD, MEMBER_CARD, IRON_RUNE,
						SILVER_CARD, WHITE_GEM, BLUE_GEM, RED_GEM));

		public static Set<Item> commonDrops = new HashSet<Item>(Arrays.asList(VULNERARY, ANTITOXIN, TORCH, MINE,
				LIGHT_RUNE, DOOR_KEY, CHEST_KEY, CHEST_KEY_5, PURE_WATER, ELIXIR, RED_GEM));
		public static Set<Item> uncommonDrops = new HashSet<Item>(Arrays.asList(HERO_CREST, KNIGHT_CREST, ORION_BOLT,
				ELYSIAN_WHIP, GUIDING_RING, EARTH_SEAL, FELL_CONTRACT, OCEAN_SEAL, AFA_DROPS, BLUE_GEM));
		...
		public static Set<Item> allWeapons = new HashSet<Item>(Arrays.asList(IRON_SWORD, SLIM_SWORD, STEEL_SWORD,
				SILVER_SWORD, IRON_BLADE, STEEL_BLADE, SILVER_BLADE, POISON_SWORD, RAPIER, MANI_KATTI, BRAVE_SWORD,
				WO_DAO, KILLING_EDGE, ARMORSLAYER, WYRMSLAYER, LIGHT_BRAND, RUNE_SWORD, LANCEREAVER, LONGSWORD,
				EMBLEM_SWORD, DURANDAL, SOL_KATTI, REGAL_BLADE, WIND_SWORD, IRON_LANCE, SLIM_LANCE, STEEL_LANCE,
				SILVER_LANCE, POISON_LANCE, BRAVE_LANCE, KILLER_LANCE, HORSESLAYER, JAVELIN, SPEAR, AXEREAVER,
				EMBLEM_LANCE, REX_HASTA, HEAVY_SPEAR, SHORT_SPEAR, IRON_AXE, STEEL_AXE, SILVER_AXE, POISON_AXE,
				BRAVE_AXE, KILLER_AXE, HALBERD, HAMMER, DEVIL_AXE, HAND_AXE, TOMAHAWK, SWORDREAVER, SWORDSLAYER,
				DRAGON_AXE, EMBLEM_AXE, ARMADS, WOLF_BEIL, BASILIKOS, IRON_BOW, STEEL_BOW, SILVER_BOW, POISON_BOW,
				KILLER_BOW, BRAVE_BOW, SHORT_BOW, LONGBOW, EMBLEM_BOW, RIENFLECHE, FIRE, THUNDER, ELFIRE, BOLTING,
				FIMBULVETR, FORBLAZE, EXCALIBUR, LIGHTNING, SHINE, DIVINE, PURGE, AURA, LUCE, AUREOLA, FLUX, LUNA,
				NOSFERATU, ECLIPSE, FENRIR, GESPENST));
		public static Set<Item> allRangedWeapons = new HashSet<Item>(
				Arrays.asList(LIGHT_BRAND, RUNE_SWORD, WIND_SWORD, JAVELIN, SPEAR, SHORT_SPEAR, HAND_AXE, TOMAHAWK,
						IRON_BOW, STEEL_BOW, SILVER_BOW, POISON_BOW, KILLER_BOW, BRAVE_BOW, SHORT_BOW, LONGBOW,
						EMBLEM_BOW, RIENFLECHE, FIRE, THUNDER, ELFIRE, FIMBULVETR, FORBLAZE, EXCALIBUR, LIGHTNING,
						SHINE, DIVINE, AURA, LUCE, AUREOLA, FLUX, LUNA, NOSFERATU, FENRIR, GESPENST));
		...
		public static Set<Item> allERank = new HashSet<Item>(Arrays.asList(IRON_SWORD, SLIM_SWORD, EMBLEM_SWORD,
				IRON_LANCE, SLIM_LANCE, JAVELIN, EMBLEM_LANCE, POISON_LANCE, HAND_AXE, IRON_AXE, EMBLEM_AXE, STEEL_AXE,
				DEVIL_AXE, IRON_BOW, EMBLEM_BOW, FIRE, LIGHTNING, HEAL));
		public static Set<Item> allDRank = new HashSet<Item>(Arrays.asList(POISON_SWORD, STEEL_SWORD, IRON_BLADE,
				ARMORSLAYER, LONGSWORD, WO_DAO, STEEL_LANCE, HEAVY_SPEAR, HORSESLAYER, POISON_AXE, HALBERD, HAMMER,
				POISON_BOW, SHORT_BOW, LONGBOW, STEEL_BOW, THUNDER, SHINE, FLUX, MEND, TORCH_STAFF, UNLOCK));
		public static Set<Item> allCRank = new HashSet<Item>(Arrays.asList(STEEL_BLADE, KILLING_EDGE, WYRMSLAYER,
				LIGHT_BRAND, LANCEREAVER, SHORT_SPEAR, KILLER_LANCE, AXEREAVER, DRAGON_AXE, KILLER_AXE, SWORDREAVER,
		...
		public static Set<Item> allBRank = new HashSet<Item>(Arrays.asList(BRAVE_SWORD, WIND_SWORD, BRAVE_LANCE, SPEAR,
				UBER_SPEAR, BRAVE_AXE, BRAVE_BOW, BOLTING, PURGE, ECLIPSE, PHYSIC, SILENCE, SLEEP, BERSERK, RESCUE));
		public static Set<Item> allARank = new HashSet<Item>(Arrays.asList(SILVER_SWORD, SILVER_BLADE, RUNE_SWORD,
				SILVER_LANCE, TOMAHAWK, SILVER_AXE, SILVER_BOW, FIMBULVETR, AURA, FENRIR, FORTIFY, WARP));
		public static Set<Item> allSRank = new HashSet<Item>(
				Arrays.asList(REGAL_BLADE, REX_HASTA, BASILIKOS, RIENFLECHE, EXCALIBUR, LUCE, GESPENST, AUREOLA));
		public static Set<Item> allPrfRank = new HashSet<Item>(
				Arrays.asList(MANI_KATTI, RAPIER, DURANDAL, SOL_KATTI, WOLF_BEIL, ARMADS, FORBLAZE));

		public static Set<Item> normalSet = new HashSet<Item>(Arrays.asList(IRON_SWORD, SLIM_SWORD, STEEL_SWORD,
				SILVER_SWORD, IRON_BLADE, STEEL_BLADE, SILVER_BLADE, EMBLEM_SWORD, IRON_LANCE, SLIM_LANCE, STEEL_LANCE,
				SILVER_LANCE, EMBLEM_LANCE, IRON_AXE, STEEL_AXE, SILVER_AXE, EMBLEM_AXE, IRON_BOW, STEEL_BOW,
				SILVER_BOW, SHORT_BOW, EMBLEM_BOW, FIRE, THUNDER, ELFIRE, FIMBULVETR, LIGHTNING, SHINE, DIVINE, AURA,
				FLUX, FENRIR, HEAL, MEND, RECOVER));
		public static Set<Item> interestingSet = new HashSet<Item>(Arrays.asList(POISON_SWORD, RAPIER, MANI_KATTI,
				BRAVE_SWORD, WO_DAO, KILLING_EDGE, ARMORSLAYER, WYRMSLAYER, LIGHT_BRAND, RUNE_SWORD, LANCEREAVER,
				LONGSWORD, WIND_SWORD, POISON_LANCE, BRAVE_LANCE, KILLER_LANCE, HORSESLAYER, JAVELIN, SPEAR, AXEREAVER,
				HEAVY_SPEAR, SHORT_SPEAR, POISON_AXE, BRAVE_AXE, KILLER_AXE, HALBERD, HAMMER, DEVIL_AXE, HAND_AXE,
				TOMAHAWK, SWORDREAVER, SWORDSLAYER, DRAGON_AXE, WOLF_BEIL, POISON_BOW, KILLER_BOW, BRAVE_BOW, LONGBOW,
				THUNDER, BOLTING, SHINE, PURGE, LUNA, NOSFERATU, ECLIPSE, PHYSIC, FORTIFY, RESTORE, WARP, RESCUE,
				TORCH_STAFF, HAMMERNE, UNLOCK, BARRIER, SILENCE, SLEEP, BERSERK));
		public static Set<Item> promoSet = new HashSet<Item>(
				Arrays.asList(EMBLEM_AXE, EMBLEM_BOW, EMBLEM_LANCE, EMBLEM_SWORD, EMBLEM_SEAL));
		...
		public static Set<Item> killerSet = new HashSet<Item>(
				Arrays.asList(KILLING_EDGE, WO_DAO, MANI_KATTI, KILLER_LANCE, KILLER_AXE, KILLER_BOW, LUNA));
		public static Set<Item> effectiveSet = new HashSet<Item>(Arrays.asList(RAPIER, MANI_KATTI, ARMORSLAYER,
				WYRMSLAYER, LONGSWORD, HORSESLAYER, HEAVY_SPEAR, HALBERD, HAMMER, SWORDSLAYER, DRAGON_AXE, WOLF_BEIL));
		public static Set<Item> poisonSet = new HashSet<Item>(
				Arrays.asList(POISON_SWORD, POISON_AXE, POISON_LANCE, POISON_BOW));
		public static Set<Item> rangedSet = new HashSet<Item>(Arrays.asList(LIGHT_BRAND, RUNE_SWORD, JAVELIN, SPEAR,
				SHORT_SPEAR, HAND_AXE, TOMAHAWK, LONGBOW, BOLTING, PURGE, ECLIPSE, PHYSIC));
		...
		public static Set<Item> allBasicWeapons = new HashSet<Item>(
				Arrays.asList(IRON_SWORD, IRON_LANCE, IRON_AXE, IRON_BOW, FIRE, LIGHTNING, FLUX));
		...
		public static Set<Item> prfWeaponsForClassID(int classID) {
			...
			} else if (classID == FE7Data.CharacterClass.LORD_ELIWOOD.ID
					|| classID == FE7Data.CharacterClass.LORD_KNIGHT.ID) {
				return new HashSet<Item>(Arrays.asList(RAPIER));
			} else if (classID == FE7Data.CharacterClass.LORD_HECTOR.ID
					|| classID == FE7Data.CharacterClass.GREAT_LORD.ID) {
		...
		public static Set<Item> lockedWeaponsToClassID(int classID) {
			if (classID == FE7Data.CharacterClass.MYRMIDON.ID || classID == FE7Data.CharacterClass.MYRMIDON_F.ID
					|| classID == FE7Data.CharacterClass.SWORDMASTER.ID
					|| classID == FE7Data.CharacterClass.SWORDMASTER_F.ID
					|| classID == FE7Data.CharacterClass.LORD_LYN.ID
					|| classID == FE7Data.CharacterClass.BLADE_LORD.ID) {
		...
		public static Set<Item> weaponsOfTypeAndRank(WeaponType type, WeaponRank min, WeaponRank max,
			...
			list.remove(WO_DAO); // This one is special. It must be added in only if we're certain the class
	...
	public enum ChapterMetadata implements GBAFEChapterMetadataChapter {
		...
		PROLOGUE_A_GIRL_FROM_THE_PLAINS(0x0), CHAPTER_1_FOOTSTEPS_OF_FATE(0x1), CHAPTER_2_SWORD_OF_SPIRITS(0x2),
		CHAPTER_3_BAND_OF_MERCENARIES(0x3), CHAPTER_4_IN_OCCUPATIONS_SHADOW(0x4), CHAPTER_5_BEYOND_THE_BORDERS(0x5),
		CHAPTER_6_BLOOD_OF_PRIDE(0x6), CHAPTER_7_SIBLINGS_ABROAD(0x7), CHAPTER_7X_THE_BLACK_SHADOW(0x8),
		CHAPTER_8_VORTEX_OF_STRATEGY(0x9), CHAPTER_9_A_GRIM_REUNION(0xA), CHAPTER_10_THE_DISTANT_PLAINS(0xB),
		CHAPTER_11E_TAKING_LEAVE(0xC), CHAPTER_11H_ANOTHER_JOURNEY(0xD), CHAPTER_12_BIRDS_OF_A_FEATHER(0xE),
		CHAPTER_13_IN_SEARCH_OF_TRUTH(0xF), CHAPTER_13X_THE_PEDDLER_MERLINUS(0x10), CHAPTER_14_FALSE_FRIENDS(0x11),
		CHAPTER_15_TALONS_ALIGHT(0x12), CHAPTER_16_NOBLE_LADY_OF_CAELIN(0x13), CHAPTER_17_WHEREABOUTS_UNKNOWN(0x14),
		CHAPTER_17X_THE_PORT_OF_BADON(0x15), CHAPTER_18_PIRATE_SHIP(0x16), CHAPTER_19_THE_DREAD_ISLE(0x17),
		CHAPTER_19X_IMPRISONER_OF_MAGIC(0x18), CHAPTER_19XX_A_GLIMPSE_IN_TIME(0x19), CHAPTER_20_DRAGONS_GATE(0x1A),
		CHAPTER_21_NEW_RESOLVE(0x1B), CHAPTER_22_KINSHIPS_BOND(0x1C), CHAPTER_23_LIVING_LEGEND(0x1D),
		CHAPTER_23X_GENESIS(0x1E), CHAPTER_24_FOUR_FANGED_OFFENSE_LLOYD(0x1F),
		CHAPTER_24_FOUR_FANGED_OFFENSE_LINUS(0x20), CHAPTER_25_CRAZED_BEAST(0x21), CHAPTER_26_UNFULFILLED_HEART(0x22),
		CHAPTER_27_PALE_FLOWER_OF_DARKNESS_KENNETH(0x23), CHAPTER_27_PALE_FLOWER_OF_DARKNESS_JERME(0x24),
		CHAPTER_28_BATTLE_BEFORE_DAWN(0x25), CHAPTER_28X_NIGHT_OF_FAREWELLS(0x26), CHAPTER_29_COG_OF_DESTINY(0x27),
		CHAPTER_28E_VALOROUS_ROLAND(0x28), CHAPTER_30H_THE_BERSERKER(0x29), CHAPTER_31_SANDS_OF_TIME(0x2A),
		CHAPTER_31X_BATTLE_PREPARATIONS(0x2B), CHAPTER_32_VICTORY_OR_DEATH(0x2C), CHAPTER_32X_THE_VALUE_OF_LIFE(0x2D),
		FINAL_CHAPTER_LIGHT_1(0x2E), FINAL_CHAPTER_LIGHT_2(0x2F);
		...
		public static ChapterMetadata[] orderedChapters() {
			return new ChapterMetadata[] { PROLOGUE_A_GIRL_FROM_THE_PLAINS, CHAPTER_1_FOOTSTEPS_OF_FATE,
					CHAPTER_2_SWORD_OF_SPIRITS, CHAPTER_3_BAND_OF_MERCENARIES, CHAPTER_4_IN_OCCUPATIONS_SHADOW,
					CHAPTER_5_BEYOND_THE_BORDERS, CHAPTER_6_BLOOD_OF_PRIDE, CHAPTER_7_SIBLINGS_ABROAD,
					CHAPTER_7X_THE_BLACK_SHADOW, CHAPTER_8_VORTEX_OF_STRATEGY, CHAPTER_9_A_GRIM_REUNION,
					CHAPTER_10_THE_DISTANT_PLAINS, CHAPTER_11E_TAKING_LEAVE, CHAPTER_11H_ANOTHER_JOURNEY,
					CHAPTER_12_BIRDS_OF_A_FEATHER, CHAPTER_13_IN_SEARCH_OF_TRUTH, CHAPTER_13X_THE_PEDDLER_MERLINUS,
					CHAPTER_14_FALSE_FRIENDS, CHAPTER_15_TALONS_ALIGHT, CHAPTER_16_NOBLE_LADY_OF_CAELIN,
					CHAPTER_17_WHEREABOUTS_UNKNOWN, CHAPTER_17X_THE_PORT_OF_BADON, CHAPTER_18_PIRATE_SHIP,
					CHAPTER_19_THE_DREAD_ISLE, CHAPTER_19X_IMPRISONER_OF_MAGIC, CHAPTER_19XX_A_GLIMPSE_IN_TIME,
					CHAPTER_20_DRAGONS_GATE, CHAPTER_21_NEW_RESOLVE, CHAPTER_22_KINSHIPS_BOND, CHAPTER_23_LIVING_LEGEND,
					CHAPTER_23X_GENESIS, CHAPTER_24_FOUR_FANGED_OFFENSE_LLOYD, CHAPTER_24_FOUR_FANGED_OFFENSE_LINUS,
					CHAPTER_25_CRAZED_BEAST, CHAPTER_26_UNFULFILLED_HEART, CHAPTER_27_PALE_FLOWER_OF_DARKNESS_KENNETH,
					CHAPTER_27_PALE_FLOWER_OF_DARKNESS_JERME, CHAPTER_28_BATTLE_BEFORE_DAWN,
					CHAPTER_28X_NIGHT_OF_FAREWELLS, CHAPTER_29_COG_OF_DESTINY, CHAPTER_28E_VALOROUS_ROLAND,
					CHAPTER_30H_THE_BERSERKER, CHAPTER_31_SANDS_OF_TIME, CHAPTER_31X_BATTLE_PREPARATIONS,
					CHAPTER_32_VICTORY_OR_DEATH, CHAPTER_32X_THE_VALUE_OF_LIFE, FINAL_CHAPTER_LIGHT_1,
					FINAL_CHAPTER_LIGHT_2 };
		}

		public String getFriendlyName() {
			switch (this) {
			case PROLOGUE_A_GIRL_FROM_THE_PLAINS:
				return "Prologue: A Girl From the Plains";
			case CHAPTER_1_FOOTSTEPS_OF_FATE:
				return "Chapter 1: Footsteps of Fate";
			case CHAPTER_2_SWORD_OF_SPIRITS:
				return "Chapter 2: Sword of Spirits";
			case CHAPTER_3_BAND_OF_MERCENARIES:
				return "Chapter 3: Band of Mercenaries";
			case CHAPTER_4_IN_OCCUPATIONS_SHADOW:
				return "Chapter 4: In Occupation's Shadow";
			case CHAPTER_5_BEYOND_THE_BORDERS:
				return "Chapter 5: Beyond the Borders";
			case CHAPTER_6_BLOOD_OF_PRIDE:
				return "Chapter 6: Blood of Pride";
			case CHAPTER_7_SIBLINGS_ABROAD:
				return "Chapter 7: Siblings Abroad";
			case CHAPTER_7X_THE_BLACK_SHADOW:
				return "Chapter 7x: The Black Shadow";
			case CHAPTER_8_VORTEX_OF_STRATEGY:
				return "Chapter 8: Vortex of Strategy";
			case CHAPTER_9_A_GRIM_REUNION:
				return "Chapter 9: A Grim Reunion";
			case CHAPTER_10_THE_DISTANT_PLAINS:
				return "Chapter 10: The Distant Plains";
			case CHAPTER_11E_TAKING_LEAVE:
				return "Chapter 11 (Eliwood): Taking Leave";
			case CHAPTER_11H_ANOTHER_JOURNEY:
				return "Chapter 11 (Hector): Another Journey";
			case CHAPTER_12_BIRDS_OF_A_FEATHER:
				return "Chapter 12: Birds of a Feather";
			case CHAPTER_13_IN_SEARCH_OF_TRUTH:
				return "Chapter 13: In Search of Truth";
			case CHAPTER_13X_THE_PEDDLER_MERLINUS:
				return "Chapter 13x: The Peddler Merlinus";
			case CHAPTER_14_FALSE_FRIENDS:
				return "Chapter 14: False Friends";
			case CHAPTER_15_TALONS_ALIGHT:
				return "Chapter 15: Talons Alight";
			case CHAPTER_16_NOBLE_LADY_OF_CAELIN:
				return "Chapter 16: Noble Lady of Caelin";
			case CHAPTER_17_WHEREABOUTS_UNKNOWN:
				return "Chapter 17: Whereabouts Unknown";
			case CHAPTER_17X_THE_PORT_OF_BADON:
				return "Chapter 17x: The Port of Badon";
			case CHAPTER_18_PIRATE_SHIP:
				return "Chapter 18: Pirate Ship";
			case CHAPTER_19_THE_DREAD_ISLE:
				return "Chapter 19: The Dread Isle";
			case CHAPTER_19X_IMPRISONER_OF_MAGIC:
				return "Chapter 19x: Imprisoner of Magic";
			case CHAPTER_19XX_A_GLIMPSE_IN_TIME:
				return "Chapter 19xx: A Glimpse in Time";
			case CHAPTER_20_DRAGONS_GATE:
				return "Chapter 20: Dragon's Gate";
			case CHAPTER_21_NEW_RESOLVE:
				return "Chapter 21: New Resolve";
			case CHAPTER_22_KINSHIPS_BOND:
				return "Chapter 22: Kinship's Bond";
			case CHAPTER_23_LIVING_LEGEND:
				return "Chapter 23: Living Legend";
			case CHAPTER_23X_GENESIS:
				return "Chapter 23x: Genesis";
			case CHAPTER_24_FOUR_FANGED_OFFENSE_LLOYD:
				return "Chapter 24: Four Fanged Offense (Lloyd)";
			case CHAPTER_24_FOUR_FANGED_OFFENSE_LINUS:
				return "Chapter 24: Four Fanged Offense (Linus)";
			case CHAPTER_25_CRAZED_BEAST:
				return "Chapter 25: Crazed Beast";
			case CHAPTER_26_UNFULFILLED_HEART:
				return "Chapter 26: Unfulfilled Heart";
			case CHAPTER_27_PALE_FLOWER_OF_DARKNESS_KENNETH:
				return "Chapter 27: Pale Flower of Darkness (Kenneth)";
			case CHAPTER_27_PALE_FLOWER_OF_DARKNESS_JERME:
				return "Chapter 27: Pale Flower of Darkness (Jerme)";
			case CHAPTER_28_BATTLE_BEFORE_DAWN:
				return "Chapter 28: Battle Before Dawn";
			case CHAPTER_28X_NIGHT_OF_FAREWELLS:
				return "Chapter 28x: Night of Farewells";
			case CHAPTER_29_COG_OF_DESTINY:
				return "Chapter 29: Cog of Destiny";
			case CHAPTER_28E_VALOROUS_ROLAND:
				return "Chapter 28 (Eliwood): Valorous Roland";
			case CHAPTER_30H_THE_BERSERKER:
				return "Chapter 30 (Hector): The Berserker";
			case CHAPTER_31_SANDS_OF_TIME:
				return "Chapter 31: Sands of Time";
			case CHAPTER_31X_BATTLE_PREPARATIONS:
				return "Chapter 31x: Battle Preparations";
			case CHAPTER_32_VICTORY_OR_DEATH:
				return "Chapter 32: Victory or Death";
			case CHAPTER_32X_THE_VALUE_OF_LIFE:
				return "Chapter 32x: The Value of Life";
			case FINAL_CHAPTER_LIGHT_1:
				return "Final: Light (Part 1)";
			case FINAL_CHAPTER_LIGHT_2:
				return "Final: Light (Part 2)";
			default:
				return "?";
			}
		}
	...
	public enum ChapterPointer {
		PROLOGUE(0x06), CHAPTER_1(0x09), CHAPTER_2(0x0F), CHAPTER_3(0x15), CHAPTER_4(0x1B), CHAPTER_5(0x21),
		CHAPTER_6(0x25), CHAPTER_7(0x28), CHAPTER_7X(0x2C), CHAPTER_8(0x2F), CHAPTER_9(0x32), CHAPTER_10(0x36),

		CHAPTER_11_E(0x39), CHAPTER_11_H(0x3D), CHAPTER_12(0x40), CHAPTER_13(0x43), CHAPTER_13X(0x47), CHAPTER_14(0x4B),
		CHAPTER_15(0x4E), CHAPTER_16(0x51), CHAPTER_17(0x55), CHAPTER_17X(0x5A), CHAPTER_18(0x61), CHAPTER_19(0x65),
		CHAPTER_19X(0x69), CHAPTER_19XX(0x6F), CHAPTER_20(0x72), CHAPTER_21(0x76), CHAPTER_22(0x7A), CHAPTER_23(0x7F),
		CHAPTER_23X(0x83), CHAPTER_24_LLOYD(0x86), CHAPTER_24_LINUS(0x89), CHAPTER_25(0x8D), CHAPTER_26(0x90),
		CHAPTER_27_KENNETH(0x96), CHAPTER_27_JERME(0x9A), CHAPTER_28(0x9E), CHAPTER_28X(0xA3), CHAPTER_29(0xA6),
		CHAPTER_28_E(0xAD), CHAPTER_30_H(0xB3), CHAPTER_31(0xB6), CHAPTER_31X(0xB9), CHAPTER_32(0xBC),
		CHAPTER_32X(0xC0), CHAPTER_FINAL(0xC7), CHAPTER_FINAL_2(0xCA);
		...
		public static List<ChapterPointer> orderedChapters() {
			return new ArrayList<ChapterPointer>(Arrays.asList(PROLOGUE, CHAPTER_1, CHAPTER_2, CHAPTER_3, CHAPTER_4,
					...
					CHAPTER_11_H, CHAPTER_12, CHAPTER_13, CHAPTER_13X, CHAPTER_14, CHAPTER_15, CHAPTER_16, CHAPTER_17,
					CHAPTER_17X, CHAPTER_18, CHAPTER_19, CHAPTER_19X, CHAPTER_19XX, CHAPTER_20, CHAPTER_21, CHAPTER_22,
					CHAPTER_23, CHAPTER_23X, CHAPTER_24_LLOYD, CHAPTER_24_LINUS, CHAPTER_25, CHAPTER_26,
					CHAPTER_27_KENNETH, CHAPTER_27_JERME, CHAPTER_28, CHAPTER_28X, CHAPTER_28_E, CHAPTER_29,
					CHAPTER_30_H, CHAPTER_31, CHAPTER_31X, CHAPTER_32, CHAPTER_32X, CHAPTER_FINAL, CHAPTER_FINAL_2));
		}

		public ChapterMetadata getMetadata() {
			switch (this) {
			case PROLOGUE:
				return ChapterMetadata.PROLOGUE_A_GIRL_FROM_THE_PLAINS;
			case CHAPTER_1:
				return ChapterMetadata.CHAPTER_1_FOOTSTEPS_OF_FATE;
			case CHAPTER_2:
				return ChapterMetadata.CHAPTER_2_SWORD_OF_SPIRITS;
			case CHAPTER_3:
				return ChapterMetadata.CHAPTER_3_BAND_OF_MERCENARIES;
			case CHAPTER_4:
				return ChapterMetadata.CHAPTER_4_IN_OCCUPATIONS_SHADOW;
			case CHAPTER_5:
				return ChapterMetadata.CHAPTER_5_BEYOND_THE_BORDERS;
			case CHAPTER_6:
				return ChapterMetadata.CHAPTER_6_BLOOD_OF_PRIDE;
			case CHAPTER_7:
				return ChapterMetadata.CHAPTER_7_SIBLINGS_ABROAD;
			case CHAPTER_7X:
				return ChapterMetadata.CHAPTER_7X_THE_BLACK_SHADOW;
			case CHAPTER_8:
				return ChapterMetadata.CHAPTER_8_VORTEX_OF_STRATEGY;
			case CHAPTER_9:
				return ChapterMetadata.CHAPTER_9_A_GRIM_REUNION;
			case CHAPTER_10:
				return ChapterMetadata.CHAPTER_10_THE_DISTANT_PLAINS;
			case CHAPTER_11_E:
				return ChapterMetadata.CHAPTER_11E_TAKING_LEAVE;
			case CHAPTER_11_H:
				return ChapterMetadata.CHAPTER_11H_ANOTHER_JOURNEY;
			case CHAPTER_12:
				return ChapterMetadata.CHAPTER_12_BIRDS_OF_A_FEATHER;
			case CHAPTER_13:
				return ChapterMetadata.CHAPTER_13_IN_SEARCH_OF_TRUTH;
			case CHAPTER_13X:
				return ChapterMetadata.CHAPTER_13X_THE_PEDDLER_MERLINUS;
			case CHAPTER_14:
				return ChapterMetadata.CHAPTER_14_FALSE_FRIENDS;
			case CHAPTER_15:
				return ChapterMetadata.CHAPTER_15_TALONS_ALIGHT;
			case CHAPTER_16:
				return ChapterMetadata.CHAPTER_16_NOBLE_LADY_OF_CAELIN;
			case CHAPTER_17:
				return ChapterMetadata.CHAPTER_17_WHEREABOUTS_UNKNOWN;
			case CHAPTER_17X:
				return ChapterMetadata.CHAPTER_17X_THE_PORT_OF_BADON;
			case CHAPTER_18:
				return ChapterMetadata.CHAPTER_18_PIRATE_SHIP;
			case CHAPTER_19:
				return ChapterMetadata.CHAPTER_19_THE_DREAD_ISLE;
			case CHAPTER_19X:
				return ChapterMetadata.CHAPTER_19X_IMPRISONER_OF_MAGIC;
			case CHAPTER_19XX:
				return ChapterMetadata.CHAPTER_19XX_A_GLIMPSE_IN_TIME;
			case CHAPTER_20:
				return ChapterMetadata.CHAPTER_20_DRAGONS_GATE;
			case CHAPTER_21:
				return ChapterMetadata.CHAPTER_21_NEW_RESOLVE;
			case CHAPTER_22:
				return ChapterMetadata.CHAPTER_22_KINSHIPS_BOND;
			case CHAPTER_23:
				return ChapterMetadata.CHAPTER_23_LIVING_LEGEND;
			case CHAPTER_23X:
				return ChapterMetadata.CHAPTER_23X_GENESIS;
			case CHAPTER_24_LLOYD:
				return ChapterMetadata.CHAPTER_24_FOUR_FANGED_OFFENSE_LLOYD;
			case CHAPTER_24_LINUS:
				return ChapterMetadata.CHAPTER_24_FOUR_FANGED_OFFENSE_LINUS;
			case CHAPTER_25:
				return ChapterMetadata.CHAPTER_25_CRAZED_BEAST;
			case CHAPTER_26:
				return ChapterMetadata.CHAPTER_26_UNFULFILLED_HEART;
			case CHAPTER_27_KENNETH:
				return ChapterMetadata.CHAPTER_27_PALE_FLOWER_OF_DARKNESS_KENNETH;
			case CHAPTER_27_JERME:
				return ChapterMetadata.CHAPTER_27_PALE_FLOWER_OF_DARKNESS_JERME;
			case CHAPTER_28:
				return ChapterMetadata.CHAPTER_28_BATTLE_BEFORE_DAWN;
			case CHAPTER_28X:
				return ChapterMetadata.CHAPTER_28X_NIGHT_OF_FAREWELLS;
			case CHAPTER_28_E:
				return ChapterMetadata.CHAPTER_28E_VALOROUS_ROLAND;
			case CHAPTER_29:
				return ChapterMetadata.CHAPTER_29_COG_OF_DESTINY;
			case CHAPTER_30_H:
				return ChapterMetadata.CHAPTER_30H_THE_BERSERKER;
			case CHAPTER_31:
				return ChapterMetadata.CHAPTER_31_SANDS_OF_TIME;
			case CHAPTER_31X:
				return ChapterMetadata.CHAPTER_31X_BATTLE_PREPARATIONS;
			case CHAPTER_32:
				return ChapterMetadata.CHAPTER_32_VICTORY_OR_DEATH;
			case CHAPTER_32X:
				return ChapterMetadata.CHAPTER_32X_THE_VALUE_OF_LIFE;
			case CHAPTER_FINAL:
				return ChapterMetadata.FINAL_CHAPTER_LIGHT_1;
			case CHAPTER_FINAL_2:
				return ChapterMetadata.FINAL_CHAPTER_LIGHT_2;
			default:
				return null;
			}
		}
		...
		// TODO: Figure out how Lucius actually works, because it's not as easy as an
		// ITGC.
		...
		public Map<Integer, List<Integer>> worldMapSpriteClassIDToCharacterIDMapping() {
			...
			switch (this) {
			case CHAPTER_1:
				map.put(CharacterClass.CAVALIER.ID,
						new ArrayList<Integer>(Arrays.asList(Character.SAIN.ID, Character.KENT.ID)));
				break;
			case CHAPTER_3:
		...
		public static ChapterPointer chapterForWorldMapEventOffset(long worldMapOffset) {
			if (worldMapOffset == 0xCE7920L) {
				return CHAPTER_1;
			} else if (worldMapOffset == 0xCE7AC0L) {
				return CHAPTER_2;
			} else if (worldMapOffset == 0xCE7BB4L) {
				return CHAPTER_3;
			} else if (worldMapOffset == 0xCE7E1CL) {
				return CHAPTER_4;
			} else if (worldMapOffset == 0xCE7F30L) {
				return CHAPTER_5;
			} else if (worldMapOffset == 0xCE8078L) {
				return CHAPTER_6;
			} else if (worldMapOffset == 0xCE821CL) {
				return CHAPTER_7;
			} else if (worldMapOffset == 0xCE833CL) {
				return CHAPTER_7X;
			} else if (worldMapOffset == 0xCE84C8L) {
				return CHAPTER_8;
			} else if (worldMapOffset == 0xCE8618L) {
				return CHAPTER_9;
			} else if (worldMapOffset == 0xCE8894L) {
				return CHAPTER_10;
			} else if (worldMapOffset == 0xCE89ECL) {
				return CHAPTER_11_E;
			} else if (worldMapOffset == 0xCECDD8L) {
				return CHAPTER_11_H;
			} else if (worldMapOffset == 0xCE8D50L) {
				return CHAPTER_12;
			} else if (worldMapOffset == 0xCE8FACL) {
				return CHAPTER_13;
			} else if (worldMapOffset == 0xCE9200L) {
				return CHAPTER_13X;
			} else if (worldMapOffset == 0xCE9408L) {
				return CHAPTER_14;
			} else if (worldMapOffset == 0xCECF0CL) {
				return CHAPTER_15;
			} else if (worldMapOffset == 0xCE9BF8L) {
				return CHAPTER_16;
			} else if (worldMapOffset == 0xCE9DD4L) {
				return CHAPTER_17;
			} else if (worldMapOffset == 0xCE9F88L) {
				return CHAPTER_17X;
			} else if (worldMapOffset == 0xCEA10CL) {
				return CHAPTER_18;
			} else if (worldMapOffset == 0xCEA754L) {
				return CHAPTER_19;
			} else if (worldMapOffset == 0xCEA8C8L) {
				return CHAPTER_19X;
			} else if (worldMapOffset == 0xCED038L) {
				return CHAPTER_19XX;
			} else if (worldMapOffset == 0xCEAA5CL) {
				return CHAPTER_20;
			} else if (worldMapOffset == 0xCEAC48L) {
				return CHAPTER_21;
			} else if (worldMapOffset == 0xCEAEA0L) {
				return CHAPTER_22;
			} else if (worldMapOffset == 0xCEB0E8L) {
				return CHAPTER_23;
			} else if (worldMapOffset == 0xCEB3ACL) {
				return CHAPTER_24_LINUS;
			} else if (worldMapOffset == 0xCEB67CL) {
				return CHAPTER_24_LLOYD;
			} else if (worldMapOffset == 0xCED188L) {
				return CHAPTER_25;
			} else if (worldMapOffset == 0xCEB94CL) {
				return CHAPTER_26;
			} else if (worldMapOffset == 0xCEBF5CL) {
				return CHAPTER_27_JERME;
			} else if (worldMapOffset == 0xCEBD20L) {
				return CHAPTER_27_KENNETH;
			} else if (worldMapOffset == 0xCEC198L) {
				return CHAPTER_28;
			} else if (worldMapOffset == 0xCEC96CL) {
				return CHAPTER_28_E;
			} else if (worldMapOffset == 0xCEC3F8L) {
				return CHAPTER_28X;
			} else if (worldMapOffset == 0xCEC5B4L) {
				return CHAPTER_29;
			} else if (worldMapOffset == 0xCED478L) {
				return CHAPTER_30_H;
			} else if (worldMapOffset == 0xCECA24L) {
				return CHAPTER_31;
			} else if (worldMapOffset == 0xCECBB0L) {
				return CHAPTER_32;
			} else if (worldMapOffset == 0xCED554L) {
				return CHAPTER_32X;
			} else {
	...
	public enum PromotionItem implements GBAFEPromotionItem {
		// The indexes here are based off of a table that starts at 0x27428 (this may
		// not be the actual start of whatever this table is).
		...
		MASTER_SEAL(0x25), FALLEN_CONTRACT(0x29),

		// These are special. I'm not sure what points to them, so they're direct.
		HECTOR_LYN_HEAVEN_SEAL(0x46), ELIWOOD_LYN_HEAVEN_SEAL(0x47), OCEAN_SEAL(0x53);
	...
	public enum Palette {
		...
		UBER_SAGE_SONIA(0x56, Character.SONIA.ID, CharacterClass.UBER_SAGE.ID, 0xFDB26C), // Not sure when this shows
																							// up, actually, but it's
																							// there.
		...
		WYVERNKNIGHT_HEATH(0x25, Character.HEATH.ID, CharacterClass.WYVERNKNIGHT.ID, 0xFD9D44), // May need hair
																								// override.
		...
		static {
			...
			spritePalettesByClass.put(CharacterClass.CORSAIR.ID, GENERIC_PIRATE.info); // Corsair doesn't have one, so
		...
		private Palette(int paletteID, int charID, int classID, long offset) {
			...
			if (charClass != null) {
				...
				case BARD:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 5, 6, 7 },
							new int[] { 11, 12, 13, 14 }, new int[] { 8, 9, 10, 4 });
					break;
				case BERSERKER:
					if (charID == Character.HAWKEYE.ID) {
						this.info = new PaletteInfo(classID, charID, offset, new int[] { 4, 5, 6 },
								new int[] { 12, 13, 14 }, new int[] {});
					} else {
				...
				case BISHOP:
					if (charID == Character.LUCIUS.ID) {
						this.info = new PaletteInfo(classID, charID, offset, new int[] { 7, 5, 6 },
								new int[] { 8, 13, 14 }, new int[] { 11, 12 });
					} else {
				...
				case CAVALIER:
					...
					break;
				case CLERIC:
				case MONK: // May need to split out Lucius as a special case. This is assuming a Lucius
							// sprite, which is unique from other monks.
				case MYRMIDON:
				case SWORDMASTER:
				case SWORDMASTER_F:
				...
				case LORD_ELIWOOD:
				case LORD_KNIGHT:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 5, 7 },
							new int[] { 11, 12, 13, 14 }, new int[] {});
					break;
				case MAGE:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 }, new int[] { 12, 13, 14 },
				...
				case NOMAD:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 },
							new int[] { 8, 4, 9, 5, 10, 11, 14 }, new int[] {});
					break;
				case NOMADTROOPER:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 },
							new int[] { 8, 2, 9, 5, 10, 11 }, new int[] {});
					break;
				case PALADIN:
					this.info = new PaletteInfo(classID, charID, offset, new int[] {}, new int[] { 8, 7, 9, 10, 11 },
							new int[] {}, new int[] {});
					break;
				case PALADIN_F:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 }, new int[] { 9, 10, 11 },
							new int[] {}); // Hair matches shield in the female.
					break;
				case PEGASUSKNIGHT:
				...
				case PIRATE:
					this.info = new PaletteInfo(classID, charID, offset, new int[] {}, new int[] { 11, 12, 13, 14 },
							new int[] {}); // Outfit/Bandana is the only color.
					break;
				case SHAMAN:
				case DRUID:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 8, 9, 10 },
							new int[] { 11, 12, 13, 14 }, new int[] {}); // Not really hair, but it matches up in the
																			// only one that matters (Canas)
					break;
				case THIEF:
				...
				case TROUBADOUR:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 },
							new int[] { 9, 8, 10, 5, 11 }, new int[] {});
					break;
				case VALKYRIE:
					this.info = new PaletteInfo(classID, charID, offset, new int[] { 6, 7 },
							new int[] { 8, 9, 10, 14, 11 }, new int[] {});
					break;
				case WARRIOR:
					this.info = new PaletteInfo(classID, charID, offset, new int[] {}, new int[] { 12, 13, 14 },
							new int[] { 7, 11 }); // No Hair. Primary is pants/helmet color. Secondary is breastplate.
					break;
				case WYVERNKNIGHT:
		...
		public static PaletteColor[] supplementaryHairColorForCharacter(int characterID) {
			...
			switch (character) {
			case MARCUS:
				return new PaletteColor[] { new PaletteColor(208, 144, 216), new PaletteColor(168, 112, 168),
						new PaletteColor(112, 80, 120), new PaletteColor(88, 64, 96) };
			case OSWIN:
				return new PaletteColor[] { new PaletteColor(152, 144, 8), new PaletteColor(120, 112, 16),
						new PaletteColor(104, 80, 16) };
			case HEATH:
				return new PaletteColor[] { new PaletteColor(56, 184, 72), new PaletteColor(32, 144, 48) };
			case VAIDA:
				return new PaletteColor[] { new PaletteColor(248, 248, 112), new PaletteColor(216, 208, 64),
						new PaletteColor(176, 152, 64) };
			default:
				return null;
			}
		}
	...
	public int chapterCount() {
		return 34; // Including lyn chapters and endgame. Gaiden chapters count as the same
					// chapter.
	}
	...
	public int[] affinityValues() {
		int[] values = new int[GBAFECharacterData.Affinity.validAffinities().length];
		int i = 0;
		for (FE7Character.Affinity affinity : GBAFECharacterData.Affinity.validAffinities()) {
	...
	public GBAFECharacterData characterDataWithData(byte[] data, long offset, Boolean hasLimitedClasses) {
		GBAFECharacterData charData = new FE7Character(data, offset, hasLimitedClasses);
	...
	public Set<GBAFEClass> targetClassesForRandomization(GBAFEClass sourceClass, boolean isForEnemy,
		...
		return new HashSet<GBAFEClass>(CharacterClass.targetClassesForRandomization(
				CharacterClass.valueOf(sourceClass.getID()), isForEnemy, excludeSource, excludeLords, excludeThieves,
				excludeSpecial, requireAttack, requiresRange, applyRestrictions, restrictGender));
	}

	public GBAFEClass correspondingMaleClass(GBAFEClass charClass) {
		switch ((FE7Data.CharacterClass) charClass) {
		case LORD_LYN:
			return FE7Data.CharacterClass.LORD_ELIWOOD;
		case BLADE_LORD:
			return FE7Data.CharacterClass.GREAT_LORD;
		case ARCHER_F:
			return FE7Data.CharacterClass.ARCHER;
		case MAGE_F:
			return FE7Data.CharacterClass.MAGE;
		case DANCER:
			return FE7Data.CharacterClass.BARD;
		case SWORDMASTER_F:
			return FE7Data.CharacterClass.SWORDMASTER;
		case SNIPER_F:
			return FE7Data.CharacterClass.SNIPER;
		case BISHOP_F:
			return FE7Data.CharacterClass.BISHOP;
		case SAGE_F:
			return FE7Data.CharacterClass.SAGE;
		case PALADIN_F:
			return FE7Data.CharacterClass.PALADIN;
		case WYVERNLORD_F:
			return FE7Data.CharacterClass.WYVERNLORD;
		case UBER_SAGE:
			return FE7Data.CharacterClass.SAGE;
		default:
			return charClass;
	...
	public GBAFEClass correspondingFemaleClass(GBAFEClass charClass) {
		...
		case LORD_ELIWOOD:
		case LORD_HECTOR:
			return FE7Data.CharacterClass.LORD_LYN;
		case ARCHER:
			return FE7Data.CharacterClass.ARCHER_F;
		case MAGE:
			return FE7Data.CharacterClass.MAGE_F;
		case BARD:
			return FE7Data.CharacterClass.DANCER;
		case GREAT_LORD:
		case LORD_KNIGHT:
			return FE7Data.CharacterClass.BLADE_LORD;
		case SWORDMASTER:
			return FE7Data.CharacterClass.SWORDMASTER_F;
		case SNIPER:
			return FE7Data.CharacterClass.SNIPER_F;
		case BISHOP:
			return FE7Data.CharacterClass.BISHOP_F;
		case SAGE:
			return FE7Data.CharacterClass.SAGE_F;
		case PALADIN:
			return FE7Data.CharacterClass.PALADIN_F;
		case WYVERNLORD:
			return FE7Data.CharacterClass.WYVERNLORD_F;
		default:
			return charClass;
		}
	}
	...
	public GBAFEClassData classDataWithData(byte[] data, long offset, GBAFEClassData demotedClass) {
		GBAFEClassData charClass = new FE7Class(data, offset);
	...
	public Set<GBAFEItem> weaponsWithStatBoosts() {
		return new HashSet<GBAFEItem>(Arrays.asList(Item.DURANDAL, Item.SOL_KATTI, Item.ARMADS, Item.FORBLAZE));
	}

	public Set<GBAFEItem> weaponsWithEffectiveness() {
		return new HashSet<GBAFEItem>(Arrays.asList(Item.RAPIER, Item.IRON_BOW, Item.HORSESLAYER, Item.HAMMER,
				Item.SWORDSLAYER, Item.DRAGON_AXE));
	}
	...
	public String statBoostStringForWeapon(GBAFEItem weapon) {
		if (weapon == Item.DURANDAL) {
			return "+5 Strength";
		}
		if (weapon == Item.SOL_KATTI) {
			return "+5 Resistance";
		}
		if (weapon == Item.ARMADS) {
			return "+5 Defense";
		}
		if (weapon == Item.FORBLAZE) {
			return "+5 Luck";
		}
	...
	public List<GBAFEClass> knightCavEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.CAVALIER, CharacterClass.CAVALIER_F,
				CharacterClass.PALADIN, CharacterClass.PALADIN_F, CharacterClass.KNIGHT, CharacterClass.KNIGHT_F,
				CharacterClass.GENERAL, CharacterClass.GENERAL_F, CharacterClass.NOMAD, CharacterClass.NOMAD_F,
				CharacterClass.NOMADTROOPER, CharacterClass.NOMADTROOPER_F, CharacterClass.TROUBADOUR,
				CharacterClass.VALKYRIE, CharacterClass.LORD_KNIGHT, CharacterClass.GREAT_LORD, CharacterClass.NONE,
				CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE // Since there are three potential Lords
																				// that can fall under the same
																				// effectiveness, we leave space for 3
																				// class IDs and a terminator.
		));
	}

	public List<GBAFEClass> knightEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.KNIGHT, CharacterClass.KNIGHT_F,
				CharacterClass.GENERAL, CharacterClass.GENERAL_F, CharacterClass.GREAT_LORD, CharacterClass.NONE,
				CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE));
	}

	public List<GBAFEClass> cavalryEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.CAVALIER, CharacterClass.CAVALIER_F,
				CharacterClass.PALADIN, CharacterClass.PALADIN_F, CharacterClass.NOMAD, CharacterClass.NOMAD_F,
				CharacterClass.NOMADTROOPER, CharacterClass.NOMADTROOPER_F, CharacterClass.TROUBADOUR,
				CharacterClass.VALKYRIE, CharacterClass.LORD_KNIGHT, CharacterClass.NONE, CharacterClass.NONE,
				CharacterClass.NONE, CharacterClass.NONE));
	}

	public List<GBAFEClass> dragonEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.FIRE_DRAGON, CharacterClass.WYVERNKNIGHT,
				CharacterClass.WYVERNKNIGHT_F, CharacterClass.WYVERNLORD, CharacterClass.WYVERNLORD_F,
				CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE));
	}

	public List<GBAFEClass> flierEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(
				Arrays.asList(CharacterClass.PEGASUSKNIGHT, CharacterClass.FALCONKNIGHT, CharacterClass.WYVERNKNIGHT,
						CharacterClass.WYVERNKNIGHT_F, CharacterClass.WYVERNLORD, CharacterClass.WYVERNLORD_F,
						CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE));
	}
	...
	public List<GBAFEClass> myrmidonEffectivenessClasses() {
		return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.MYRMIDON, CharacterClass.MYRMIDON_F,
				...
				CharacterClass.MERCENARY_F, CharacterClass.HERO, CharacterClass.HERO_F, CharacterClass.BLADE_LORD,
				CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE, CharacterClass.NONE));
	}
...
//		if (effectivenessPtr == 0xC97ECBL) { return AdditionalData.DRAGON_EFFECT; } // Used for Aureola. It's Dragon + Dark Druid.
	...
	public List<GBAFEClass> additionalClassesForPromotionItem(GBAFEPromotionItem promotionItem,
		...
		if (promotionItem == PromotionItem.KNIGHT_CREST) {
			return new ArrayList<GBAFEClass>(
					Arrays.asList(CharacterClass.SOLDIER, CharacterClass.LORD_ELIWOOD, CharacterClass.LORD_HECTOR));
		}
		if (promotionItem == PromotionItem.HERO_CREST) {
			return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.LORD_LYN, CharacterClass.BRIGAND));
		}
		if (promotionItem == PromotionItem.MASTER_SEAL) {
			return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.SOLDIER, CharacterClass.LORD_ELIWOOD,
					CharacterClass.LORD_HECTOR, CharacterClass.LORD_LYN));
		}
		if (promotionItem == PromotionItem.OCEAN_SEAL) {
			return new ArrayList<GBAFEClass>(Arrays.asList(CharacterClass.CORSAIR, CharacterClass.BRIGAND));
		}
