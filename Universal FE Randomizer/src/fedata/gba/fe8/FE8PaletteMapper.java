...
public class FE8PaletteMapper {
	private static final DebugPrinter LOGGER_RECYCLE = DebugPrinter.forKey(DebugPrinter.Key.PALETTE_RECYCLER);
	private static final DebugPrinter LOGGER_PALETTE = DebugPrinter.forKey(DebugPrinter.Key.PALETTE);
	...
	private void markPaletteIDAsFree(int paletteID) {
		...
		LOGGER_RECYCLE.log( "Freed up palette with ID 0x" + Integer.toHexString(paletteID) + "(size: " + Integer.toString(length) + ")");
		if (length != null) {
	...
	private Integer requestRecycledPaletteForSize(int paletteSize) {
		if (recycledPaletteIDsByLength.keySet().stream().filter(size -> (size >= paletteSize)).min(WhyDoesJavaNotHaveThese.ascendingIntegerComparator).isPresent()) {
			...
			if (availableIDs.isEmpty()) {
				...
				LOGGER_RECYCLE.log( "Recycled palette with ID 0x" + Integer.toHexString(availableIDs.get(0)) + "(size: " + Integer.toString(length) + ", requested size: " + Integer.toString(paletteSize) + ")");
	...
	public Map<SlotType, Integer> requestRecycledPaletteIndicesForCharacter(int characterID) {
		...
		LOGGER_RECYCLE.log( "Handing out free palettes to character 0x" + Integer.toHexString(characterID));
		...
		if (palettesNeeded.size() < emptyPaletteIDs.size()) {
			...
			while (!slotsRemaining.isEmpty()) {
				...
				LOGGER_RECYCLE.log( "Handed out palette index 0x" + Integer.toHexString(paletteIndex));
	...
	public void setUnpromotedClass(int unpromotedClassID, int characterID, Boolean setPromotions, int basePaletteSize, int promotedPaletteSize, int secondPromotionPaletteSize) {
		...
		LOGGER_PALETTE.log( "Assigning unpromoted class ID 0x" + Integer.toHexString(unpromotedClassID) + " (" + unpromotedClass.toString() + ") to character 0x" + Integer.toHexString(characterID) + " (" + character.toString() + ")");
		...
		if (existingPaletteMap.getBasePaletteID() == 0 || (paletteLength != null && basePaletteSize > paletteLength)) {
			if (existingPaletteMap.getBasePaletteID() != 0) {
				...
				LOGGER_PALETTE.log( "Current palette (0x" + Integer.toHexString(existingPaletteMap.getBasePaletteID()) + ") is too small, looking for alternatives...");
			...
			if (recycledPaletteID != null) {
				...
				LOGGER_PALETTE.log( "Found recycled palette 0x" + Integer.toHexString(recycledPaletteID));
			} else {
				Integer emptyID = requestEmptyPaletteForSize(basePaletteSize);
				if (emptyID != null) {
					existingPaletteMap.setBasePaletteID(emptyID);
					LOGGER_PALETTE.log( "Using empty palette 0x" + Integer.toHexString(emptyID));
				} else {
					palettesNeeded.add(SlotType.PRIMARY_BASE);
					LOGGER_PALETTE.log( "No Palettes currently available. Adding to waitlist.");
				}
			}
		} else {
			LOGGER_PALETTE.log( "Palette (0x" + Integer.toHexString(existingPaletteMap.getBasePaletteID()) + ") OK! (oldSize: " + Integer.toString(paletteLength) + " newSize: " + Integer.toString(basePaletteSize) + ")");
		}
			...
			if (primaryPromotionID != 0) {
				...
				if (existingPaletteMap.getFirstPromotionPaletteID() == 0 || (paletteLength != null && promotedPaletteSize > paletteLength)) {
					if (existingPaletteMap.getFirstPromotionPaletteID() != 0) {
						LOGGER_PALETTE.log( "Current palette (0x" + Integer.toHexString(existingPaletteMap.getFirstPromotionPaletteID()) + ") is too small or doesn't exist, looking for alternatives...");
					...
					LOGGER_PALETTE.log( "Palette (0x" + Integer.toHexString(existingPaletteMap.getFirstPromotionPaletteID()) + ") OK! (oldSize: " + Integer.toString(paletteLength) + " newSize: " + Integer.toString(promotedPaletteSize) + ")");
				}
			} else {
			...
			if (secondaryPromotionID != 0) {
				...
				if (existingPaletteMap.getSecondaryPromotionPaletteID() == 0 || (paletteLength != null && secondPromotionPaletteSize > paletteLength)) {
					if (existingPaletteMap.getSecondaryPromotionPaletteID() != 0) {
						LOGGER_PALETTE.log( "Current palette (0x" + Integer.toHexString(existingPaletteMap.getSecondaryPromotionPaletteID()) + ") is too small or doesn't exist, looking for alternatives...");
					...
					LOGGER_PALETTE.log( "Palette (0x" + Integer.toHexString(existingPaletteMap.getSecondaryPromotionPaletteID()) + ") OK! (oldSize: " + Integer.toString(paletteLength) + " newSize: " + Integer.toString(secondPromotionPaletteSize) + ")");
				}
			} else {
	...
	public void setPromotedClass(int promotedClassID, int characterID, int paletteSize) {
		...
		LOGGER_PALETTE.log( "Assigning promoted class ID 0x" + Integer.toHexString(promotedClassID) + " (" + promotedClass.toString() + ") to character 0x" + Integer.toHexString(characterID) + " (" + character.toString() + ")");
		...
		if (existingPaletteMap.getFirstPromotionPaletteID() == 0 || (paletteLength != null && paletteSize > paletteLength)) {
			if (existingPaletteMap.getFirstPromotionPaletteID() != 0) {
				LOGGER_PALETTE.log( "Current palette (0x" + Integer.toHexString(existingPaletteMap.getFirstPromotionPaletteID()) + ") is too small, looking for alternatives...");
				markPaletteIDAsFree(existingPaletteMap.getFirstPromotionPaletteID());
				existingPaletteMap.setFirstPromotionPaletteID(0);
			}
			...
			LOGGER_PALETTE.log( "Palette (0x" + Integer.toHexString(existingPaletteMap.getFirstPromotionPaletteID()) + ") OK! (oldSize: " + Integer.toString(paletteLength) + " newSize: " + Integer.toString(paletteSize) + ")");
		}
