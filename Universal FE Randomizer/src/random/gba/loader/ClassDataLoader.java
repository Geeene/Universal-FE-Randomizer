...
public class ClassDataLoader {
	...
	public ClassDataLoader(GBAFEClassProvider provider, FileHandler handler) {
		...
		// This is done in two passes to satisfy FE6's lack of distinct promotion bonus
		// (which is simply the delta between the two class bases.)
	...
	public GBAFEClassData createLordClassBasedOnClass(GBAFEClassData referenceClass, int idToReplace) {
		...
		if (idToReplace >= provider.numberOfClasses()) {
			return null;
		}
		GBAFEClassData clonedClass = referenceClass.createClone();
		...
		if (addedClasses.isEmpty()) {
			for (GBAFEClassData charClass : classMap.values()) {
				...
				if (charClass.hasCommittedChanges()) {
					Diff charDiff = new Diff(charClass.getAddressOffset(), charClass.getData().length,
							charClass.getData(), null);
			...
			for (int i = 0; i < provider.numberOfClasses(); i++) {
				...
				if (objectData != null) {
					...
					long writtenOffset = freeSpace.setValue(objectData.getData(),
							"Class Data for Class 0x" + Integer.toHexString(i), i == 0);
					if (i == 0) {
						newStartingOffset = writtenOffset;
					}
				} else {
					...
					long writtenOffset = freeSpace.setValue(
							FileReadHelper.readBytesInRange(new AddressRange(existingStart, existingEnd), handler),
							"Copied class data for Class 0x" + Integer.toHexString(i), i == 0);
					if (i == 0) {
						newStartingOffset = writtenOffset;
					}
				}
			...
			for (GBAFEClassData charClass : addedClassList) {
				freeSpace.setValue(charClass.getData(),
						"Added Class Data for Class 0x" + Integer.toHexString(charClass.getID()));
			}

			// Update the pointers to this table.
			compiler.findAndReplace(new FindAndReplace(WhyDoesJavaNotHaveThese.gbaAddressFromOffset(startingOffset),
					WhyDoesJavaNotHaveThese.gbaAddressFromOffset(newStartingOffset), true));
		}
	}
	...
	public GBAFEClassData[] potentialClasses(GBAFEClassData sourceClass, Boolean isForEnemy, Boolean excludeLords,
			Boolean excludeThieves, Boolean excludeSpecial, Boolean excludeSource, Boolean requireAttack,
			Boolean requireRange, Boolean requireMelee, Boolean applyRestrictions,
			ClassOptions.GenderRestrictionOption restrictGender, GBAFEClassData mustLoseToClass) {
		return potentialClasses(sourceClass, isForEnemy, excludeLords, excludeThieves, excludeSpecial, false,
				excludeSource, requireAttack, requireRange, requireMelee, applyRestrictions, restrictGender,
				mustLoseToClass);
	}

	public GBAFEClassData[] potentialClasses(GBAFEClassData sourceClass, Boolean isForEnemy, Boolean excludeLords,
			Boolean excludeThieves, Boolean excludeSpecial, Boolean separateMonsters, Boolean excludeSource,
			Boolean requireAttack, Boolean requireRange, Boolean requireMelee, Boolean applyRestrictions,
			ClassOptions.GenderRestrictionOption restrictGender, GBAFEClassData mustLoseToClass) {
		...
		if (mustLoseToClass != null) {
			targetClasses = provider.classesThatLoseToClass(provider.classWithID(sourceClass.getID()),
					provider.classWithID(mustLoseToClass.getID()), options);
		}
	...
	public List<GBAFEClassData> demotionOptions(int classID) {
		GBAFEClass charClass = provider.classWithID(classID);
		if (charClass == null) {
			return new ArrayList<GBAFEClassData>();
		}
	...
	public List<WeaponType> usableTypesForClass(GBAFEClassData charClass) {
		...
		if (charClass.getSwordRank() > 0) {
			types.add(WeaponType.SWORD);
		}
		if (charClass.getLanceRank() > 0) {
			types.add(WeaponType.LANCE);
		}
		if (charClass.getAxeRank() > 0) {
			types.add(WeaponType.AXE);
		}
		if (charClass.getBowRank() > 0) {
			types.add(WeaponType.BOW);
		}
		if (charClass.getAnimaRank() > 0) {
			types.add(WeaponType.ANIMA);
		}
		if (charClass.getDarkRank() > 0) {
			types.add(WeaponType.DARK);
		}
		if (charClass.getLightRank() > 0) {
			types.add(WeaponType.LIGHT);
		}
		if (charClass.getStaffRank() > 0) {
	...
	private GBAFEClassData[] feClassesFromSet(Set<GBAFEClass> classes) {
		...
		Collections.sort(charClasses, new Comparator<GBAFEClass>() {
			public int compare(GBAFEClass arg0, GBAFEClass arg1) {
				return Integer.compare(arg0.getID(), arg1.getID());
			}
		});
	...
	public void recordClasses(RecordKeeper rk, Boolean isInitial, ClassDataLoader classData, TextLoader textData,
			PromotionDataLoader promotionData) {
		for (GBAFEClassData charClass : allClasses()) {
			if (!isValidClass(charClass.getID())) {
				continue;
			}

			List<Integer> promoNameindices = getPromotionNameIndeces(classData, promotionData, charClass);

			recordClass(rk, charClass, isInitial, textData, promoNameindices);
		}
	}
	 ...
	 */
	private List<Integer> getPromotionNameIndeces(ClassDataLoader classData, PromotionDataLoader promotionData,
			GBAFEClassData charClass) {
		List<Integer> promoNameIndices = new ArrayList<>();
		if (!classData.canClassPromote(charClass.getID()))
			return promoNameIndices;
		if (charClass instanceof FE8Class) {
			...
			promoNameIndices.add(classData.classForID(branch.getFirstPromotion()).getNameIndex());
			promoNameIndices.add(classData.classForID(branch.getSecondPromotion()).getNameIndex());
		} else {
			promoNameIndices.add(classData.classForID(charClass.getTargetPromotionID()).getNameIndex());
		}
		return promoNameIndices;
	}
	...
	private void recordClass(RecordKeeper rk, GBAFEClassData charClass, Boolean isInitial, TextLoader textData,
		...
		if (isInitial) {
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "HP Growth",
					String.format("%d%%", charClass.getHPGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "STR/MAG Growth",
					String.format("%d%%", charClass.getSTRGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "SKL Growth",
					String.format("%d%%", charClass.getSKLGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "SPD Growth",
					String.format("%d%%", charClass.getSPDGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "LCK Growth",
					String.format("%d%%", charClass.getLCKGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "DEF Growth",
					String.format("%d%%", charClass.getDEFGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "RES Growth",
					String.format("%d%%", charClass.getRESGrowth()));
			rk.recordOriginalEntry(RecordKeeperCategoryKey, name, "Movement Range",
					Integer.toString(charClass.getMOV()));
			...
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "HP Growth",
					String.format("%d%%", charClass.getHPGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "STR/MAG Growth",
					String.format("%d%%", charClass.getSTRGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "SKL Growth",
					String.format("%d%%", charClass.getSKLGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "SPD Growth",
					String.format("%d%%", charClass.getSPDGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "LCK Growth",
					String.format("%d%%", charClass.getLCKGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "DEF Growth",
					String.format("%d%%", charClass.getDEFGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "RES Growth",
					String.format("%d%%", charClass.getRESGrowth()));
			rk.recordUpdatedEntry(RecordKeeperCategoryKey, name, "Movement Range",
					Integer.toString(charClass.getMOV()));
			for (int i = 0; i < promos.size(); i++) {
				String promoName = textData.getStringAtIndex(promos.get(i), true).trim();
	...
	public HashMap<Integer, GBAFEClassData> getClassMap() {
