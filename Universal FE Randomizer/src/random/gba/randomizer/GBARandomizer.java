...
public class GBARandomizer extends Randomizer {
	...
	public GBARandomizer(String sourcePath, String targetPath, FEBase.GameType gameType, DiffCompiler diffs,
			GrowthOptions growths, BaseOptions bases, ClassOptions classes, PromotionOptions promos,
			WeaponOptions weapons, OtherCharacterOptions other, EnemyOptions enemies, MiscellaneousOptions otherOptions,
		...
		if (itemAssignmentOptions == null) {
			itemAssignmentOptions = new ItemAssignmentOptions(WeaponReplacementPolicy.ANY_USABLE,
					ShopAdjustment.NO_CHANGE, false, false);
		}
	...
	private void randomize(String seed) {
		...
		case FE6:
			recordKeeper
					.addNote("Characters that randomize into the Soldier class can promote using a Knight's Crest.");
			recordKeeper
					.addNote("Characters that randomize into the Roy Lord class can promote using a Knight's Crest.");
			break;
		case FE7:
		...
		case FE8:
			recordKeeper.addNote(
					"Characters that randomize into the Soldier class can promote into a Paladin or General using a Knight's Crest or Master Seal.");
			recordKeeper.addNote(
					"Characters that randomize into the Eirika Lord class can promote using a Knight's Crest or Master Seal.");
			recordKeeper.addNote(
					"Characters that randomize into the Ephraim Lord class can promote using a Knight's Crest or Master Seal.");
			recordKeeper.addNote(
					"Characters that randomize into Revenant, Sword/Lance Bonewalkers, and Mauthe Doogs promote using a Hero's Crest or Master Seal.");
			recordKeeper.addNote(
					"Characters that randomize into Tarvos and Bael promote using a Knight's Crest or Master Seal.");
			recordKeeper
					.addNote("Characters that randomize into a Mogall promote using a Guiding Ring or Master Seal.");
			recordKeeper.addNote(
					"Characters that randomize into a Bow Bonewalker promote using an Orion's Bolt or Master Seal.");
			recordKeeper
					.addNote("Characters that randomize into a Gargoyle promote using an Elysian Whip or Master Seal.");
			break;
		default:
			break;
		}
	...
	private void makeFinalAdjustments(String seed) {
		...
		// Fix promotions so that forcing a promoted unit to promote again doesn't
		// demote them.
		...
		if (gameType == GameType.FE7) {
			try {
				...
				diffCompiler.addDiff(new Diff(FE7Data.HardcodedSRAMHeaderOffset, 4,
						WhyDoesJavaNotHaveThese.bytesFromAddress(pointer),
			...
			if ((recruitOptions != null && recruitOptions.includeLords) || (classes != null && classes.includeLords)) {
				...
				diffCompiler.addDiff(new Diff(FE7Data.ModeSelectPortraitOffset, 12, new byte[] { lynReplacementFaceID,
						0, 0, 0, eliwoodReplacementFaceID, 0, 0, 0, hectorReplacementFaceID, 0, 0, 0 }, null));
				...
				diffCompiler.addDiff(new Diff(FE7Data.ModeSelectClassAnimationOffset, 12,
						new byte[] { lynReplacementAnimationID, 0, 0, 0, eliwoodReplacementAnimationID, 0, 0, 0,
								hectorReplacementAnimationID, 0, 0, 0 },
						null));

				// See if we can apply their palettes to the class default.
				PaletteHelper.applyCharacterPaletteToSprite(GameType.FE7, handler,
				...
				textData.setStringAtIndex(FE7Data.ModeSelectTextLynWeaponTypeIndex,
						lynClass.primaryWeaponType() + "[X]");
			...
			if (gameType == GameType.FE7) {
				for (FE7Data.ChapterPointer chapter : FE7Data.ChapterPointer.values()) {
					...
					for (GBAFEWorldMapSpriteData sprite : worldMapData.allSprites()) {
						...
						Integer characterID = FE7Data.ChapterPointer
								.universalWorldMapSpriteClassIDToCharacterIDMapping().get(sprite.getClassID());
						if (characterID != null) {
				...
				for (FE8Data.ChapterPointer chapter : FE8Data.ChapterPointer.values()) {
					...
					for (GBAFEWorldMapSpriteData sprite : worldMapData.allSprites()) {
						...
						// Check Universal list first.
						Integer characterID = FE8Data.ChapterPointer
								.universalWorldMapSpriteClassIDToCharacterIDMapping().get(sprite.getClassID());
						if (characterID != null) {
		...
		if (gameType == GameType.FE8) {
			...
			long offset = freeSpace
					.setValue(
							new byte[] { (byte) FE8Data.CharacterClass.TRAINEE.ID,
									(byte) FE8Data.CharacterClass.PUPIL.ID, (byte) FE8Data.CharacterClass.RECRUIT.ID },
							"TraineeSeal");
			diffCompiler.addDiff(
					new Diff(FE8Data.HeavenSealPromotionPointer, 4, WhyDoesJavaNotHaveThese.bytesFromAddress(offset),
							WhyDoesJavaNotHaveThese.bytesFromAddress(FE8Data.HeavenSealOldAddress)));
		...
		for (GBAFEChapterData chapter : chapterData.allChapters()) {
			for (GBAFEChapterUnitData chapterUnit : chapter.allUnits()) {
				...
				if (unitClass.getStaffRank() != 0) {
					if (itemData.isHealingStaff(chapterUnit.getItem1())
							|| itemData.isHealingStaff(chapterUnit.getItem2())
		...
		// Adjust Wire and/or Hector in FE7, since he has a high chance of softlocking
		// Hector and Matthew in Ch. 11.
		...
		// Do this last, so that we don't mess up anything else that needs to read class
		// IDs.
		if (gameType == GameType.FE6) {
			...
			byte[] spriteTable = handler.readBytesAtOffset(mapSpriteTableOffset,
			...
			diffCompiler
					.findAndReplace(new FindAndReplace(WhyDoesJavaNotHaveThese.bytesFromAddress(mapSpriteTableOffset),
			...
			// GBAFE only stores 5 bits for the class (in save data), so using any ID
			// greater than 0x7F will have issues. We have to replace an existing class.
			GBAFEClassData newEirikaClass = classData.createLordClassBasedOnClass(classData.classForID(oldEirikaClass),
					FE8Data.CharacterClass.UNUSED_TENT.ID); // This was a (unused?) tent.
			GBAFEClassData newEphraimClass = classData.createLordClassBasedOnClass(
					classData.classForID(oldEphraimClass), FE8Data.CharacterClass.UNUSED_MANAKETE.ID); // This is an
																										// unused
																										// manakete
																										// class.
			...
			itemData.replaceClassesForPromotionItem(FE8Data.PromotionItem.LUNAR_BRACE,
					new ArrayList<Integer>(Arrays.asList(newEirikaClass.getID())));
			itemData.replaceClassesForPromotionItem(FE8Data.PromotionItem.SOLAR_BRACE,
					new ArrayList<Integer>(Arrays.asList(newEphraimClass.getID())));
				...
				for (GBAFEChapterUnitData unit : chapter.allUnits()) {
					if (unit.getCharacterNumber() == FE8Data.Character.EIRIKA.ID) {
						...
						if (unit.getStartingClass() == oldEphraimClass) {
							unit.setStartingClass(newEphraimClass.getID());
							/* unit.setStartingLevel(10); */}
					}
					/*
					 ...
					 * unit.giveItem(itemData.itemsToPromoteClass(newEphraimClass.getID()).get(0).
					 * getID()); }
					 */
				}
			}
			...
			fe8_promotionManager.setFirstPromotionOptionForClass(newEirikaClass.getID(),
					fe8_promotionManager.getFirstPromotionOptionClassID(oldEirikaClass));
			fe8_promotionManager.setSecondPromotionOptionForClass(newEirikaClass.getID(),
					fe8_promotionManager.getSecondPromotionOptionClassID(oldEirikaClass));
			fe8_promotionManager.setFirstPromotionOptionForClass(newEphraimClass.getID(),
					fe8_promotionManager.getFirstPromotionOptionClassID(oldEphraimClass));
			fe8_promotionManager.setSecondPromotionOptionForClass(newEphraimClass.getID(),
					fe8_promotionManager.getSecondPromotionOptionClassID(oldEphraimClass));

			// Palettes are also tied to class.
			FE8PaletteMapper.ClassMapEntry eirikaPalette = fe8_paletteMapper
					.getEntryForCharacter(FE8Data.Character.EIRIKA);
			FE8PaletteMapper.ClassMapEntry ephraimPalette = fe8_paletteMapper
					.getEntryForCharacter(FE8Data.Character.EPHRAIM);
			...
			// On the bright side, we don't need to repoint the FE8 map sprite table. We
			// just need to replace some entries in the existing one.
			...
			if (fe8_walkingSoundFixApplied) {
				...
				diffCompiler
						.addDiff(new Diff(ephraimWalkingSoundOffset, 1, new byte[] { ephraimWalkingSoundID }, null));
			}
		}
			...
			if (gameType == GameType.FE6) {
				...
				if (!royWeaponTypes.isEmpty()) {
					...
					if (weaponName != null && iconName != null) {
						...
						GBAFEClassData oldPromotedLordClass = classData
								.classForID(FE6Data.CharacterClass.MASTER_LORD.ID);
				...
				if (lynSelectedType != null && lynWeaponName != null && lynIconName != null) {
					...
					GBAFEItemData newWeapon = referenceWeapon.createLordWeapon(FE7Data.Character.LYN.ID, 0x9F, 0x1225,
							0x0, lynSelectedType, unbreakablePrfs, lynClass.getCON() + lyn.getConstitution(), 0xAD,
							itemData, freeSpace);
					...
					case DARK:
						itemData.spellAnimations.addAnimation(newWeapon.getID(), 2,
								FE7SpellAnimationCollection.Animation.FLUX.value,
								FE7SpellAnimationCollection.Flash.DARK.value);
						break;
					case LIGHT:
						itemData.spellAnimations.addAnimation(newWeapon.getID(), 2,
					...
					for (GBAFEChapterData chapter : chapterData.allChapters()) {
						if (chapter == chapterData.chapterWithID(FE7Data.ChapterPointer.CHAPTER_2.chapterID)) {
							GBAFEChapterItemData item = chapter
									.chapterItemGivenToCharacter(FE7Data.Character.LYN_TUTORIAL.ID);
							if (item != null) {
				...
				if (eliwoodSelectedType != null && eliwoodWeaponName != null && eliwoodIconName != null) {
					...
					GBAFEItemData newWeapon = referenceWeapon.createLordWeapon(FE7Data.Character.ELIWOOD.ID, 0xA0,
							0x1227, 0x0, eliwoodSelectedType, unbreakablePrfs,
							eliwoodClass.getCON() + eliwood.getConstitution(), 0xAE, itemData, freeSpace);
					...
					if (eliwoodSelectedType == WeaponType.SWORD) {
						if (!athosLockUsed) {
							...
							eliwood.enableWeaponLock(
									FE7Data.CharacterAndClassAbility3Mask.UNUSED_WEAPON_LOCK.getValue());
						}
				...
				if (hectorSelectedType != null && hectorWeaponName != null && hectorIconName != null) {
					...
					GBAFEItemData newWeapon = referenceWeapon.createLordWeapon(FE7Data.Character.HECTOR.ID, 0xA1,
							0x1229, 0x0, hectorSelectedType, unbreakablePrfs,
							hectorClass.getCON() + hector.getConstitution(), 0xAF, itemData, freeSpace);
				...
				if (eirikaWeaponName != null && eirikaIconName != null) {
					// Replace the old icon.
					byte[] iconData = GBAImageCodec.getGBAGraphicsDataForImage(eirikaIconName,
							GBAImageCodec.gbaWeaponColorPalette);
					if (iconData == null) {
					...
					// We need a description string so that the rest of the weapon stats will show,
					...
					itemToReplace.turnIntoLordWeapon(eirika.getID(), 0x3A, 0x3B, eirikaSelectedType, unbreakablePrfs,
							eirikaClass.getCON() + eirika.getConstitution(),
					...
					case AXE:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.NONE2.value,
								FE8SpellAnimationCollection.Flash.WHITE.value);
						break;
					case BOW:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.ARROW.value,
								FE8SpellAnimationCollection.Flash.WHITE.value);
						break;
					case ANIMA:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.THUNDER.value,
								FE8SpellAnimationCollection.Flash.YELLOW.value);
						break;
					case DARK:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.FLUX.value,
								FE8SpellAnimationCollection.Flash.DARK.value);
						break;
					case LIGHT:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.DIVINE.value,
								FE8SpellAnimationCollection.Flash.BLUE.value);
						break;
					default:
						// No animation needed here.
						break;
				...
				if (ephraimWeaponName != null && ephraimIconName != null) {
					// Replace the old icon.
					byte[] iconData = GBAImageCodec.getGBAGraphicsDataForImage(ephraimIconName,
							GBAImageCodec.gbaWeaponColorPalette);
					if (iconData == null) {
					...
					itemToReplace.turnIntoLordWeapon(eirika.getID(), 0x3C, 0x3D, ephraimSelectedType, unbreakablePrfs,
							ephraimClass.getCON() + ephraim.getConstitution(),
					...
					case ANIMA:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.ELFIRE.value,
								FE8SpellAnimationCollection.Flash.RED.value);
						break;
					case DARK:
						itemData.spellAnimations.addAnimation(itemToReplace.getID(), 2,
								FE8SpellAnimationCollection.Animation.FLUX.value,
	...
	public RecordKeeper initializeRecordKeeper() {
		...
		if (growths != null) {
			switch (growths.mode) {
			case REDISTRIBUTE:
				rk.addHeaderItem("Randomize Growths",
						"Redistribution (" + growths.redistributionOption.variance + "% variance)");
				break;
			case DELTA:
			...
			case FULL:
				rk.addHeaderItem("Randomize Growths",
						"Full (" + growths.fullOption.minValue + "% ~ " + growths.fullOption.maxValue + "%)");
				break;
			}
		...
		if (bases != null) {
			switch (bases.mode) {
			case REDISTRIBUTE:
				rk.addHeaderItem("Randomize Bases",
						"Redistribution (" + bases.redistributionOption.variance + " variance)");
				break;
			case DELTA:
		...
		if (otherCharacterOptions.constitutionOptions != null) {
			rk.addHeaderItem("Randomize Constitution", "+/- " + otherCharacterOptions.constitutionOptions.variance
					+ ", Min: " + otherCharacterOptions.constitutionOptions.minValue);
		} else {
			rk.addHeaderItem("Randomize Constitution", "NO");
		}

		if (otherCharacterOptions.movementOptions != null) {
			rk.addHeaderItem("Randomize Movement Ranges", "" + otherCharacterOptions.movementOptions.minValue + " ~ "
					+ otherCharacterOptions.movementOptions.maxValue);
		} else {
		...
		if (weapons.mightOptions != null) {
			rk.addHeaderItem("Randomize Weapon Power", "+/- " + weapons.mightOptions.variance + ", ("
					+ weapons.mightOptions.minValue + " ~ " + weapons.mightOptions.maxValue + ")");
		} else {
			rk.addHeaderItem("Randomize Weapon Power", "NO");
		}
		if (weapons.hitOptions != null) {
			rk.addHeaderItem("Randomize Weapon Accuracy", "+/- " + weapons.hitOptions.variance + ", ("
					+ weapons.hitOptions.minValue + " ~ " + weapons.hitOptions.maxValue + ")");
		} else {
			rk.addHeaderItem("Randomize Weapon Accuracy", "NO");
		}
		if (weapons.weightOptions != null) {
			rk.addHeaderItem("Randomize Weapon Weight", "+/- " + weapons.weightOptions.variance + ", ("
					+ weapons.weightOptions.minValue + " ~ " + weapons.weightOptions.maxValue + ")");
		} else {
			rk.addHeaderItem("Randomize Weapon Weight", "NO");
		}
		if (weapons.durabilityOptions != null) {
			rk.addHeaderItem("Randomize Weapon Durability", "+/- " + weapons.durabilityOptions.variance + ", ("
					+ weapons.durabilityOptions.minValue + " ~ " + weapons.durabilityOptions.maxValue + ")");
		} else {
		...
		if (weapons.shouldAddEffects) {
			...
			if (weapons.effectsList.statBoosts > 0) {
				sb.append("<li>Stat Boosts" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.statBoosts / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.effectiveness > 0) {
				sb.append("<li>Effectiveness" + String.format(" (%.2f%%)", (double) weapons.effectsList.effectiveness
						/ (double) weapons.effectsList.getWeightTotal() * 100) + "</li>\n");
			}
			if (weapons.effectsList.unbreakable > 0) {
				sb.append("<li>Unbreakable" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.unbreakable / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.brave > 0) {
				sb.append("<li>Brave" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.brave / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.reverseTriangle > 0) {
				sb.append(
						"<li>Reverse Triangle" + String.format(" (%.2f%%)", (double) weapons.effectsList.reverseTriangle
								/ (double) weapons.effectsList.getWeightTotal() * 100) + "</li>\n");
			}
			if (weapons.effectsList.extendedRange > 0) {
				sb.append("<li>Extended Range" + String.format(" (%.2f%%)", (double) weapons.effectsList.extendedRange
						/ (double) weapons.effectsList.getWeightTotal() * 100) + "</li>\n");
			}
			if (weapons.effectsList.highCritical > 0) {
				sb.append("<li>Critical");
				sb.append(" (" + weapons.effectsList.criticalRange.minValue + "% ~ "
						+ weapons.effectsList.criticalRange.maxValue + "%)");
				sb.append(String.format(" (%.2f%%)", (double) weapons.effectsList.highCritical
						/ (double) weapons.effectsList.getWeightTotal() * 100));
				sb.append("</li>\n");
			}
			if (weapons.effectsList.magicDamage > 0) {
				sb.append("<li>Magic Damage" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.magicDamage / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.poison > 0) {
				sb.append("<li>Poison" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.poison / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.eclipse > 0) {
				sb.append("<li>Eclipse" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.eclipse / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			if (weapons.effectsList.devil > 0) {
				sb.append("<li>Devil" + String.format(" (%.2f%%)",
						(double) weapons.effectsList.devil / (double) weapons.effectsList.getWeightTotal() * 100)
						+ "</li>\n");
			}
			sb.append("</ul>\n");
		...
		case SCALING:
			rk.addHeaderItem("Buff Minions",
					"Scaling Buff (Growths x" + String.format("%.2f", (enemies.minionBuff / 100.0) + 1) + ")");
			rk.addHeaderItem("Buffed Minion Stats", enemies.minionBuffStats.buffString());
			break;
		}
		...
		if (miscOptions.randomizeFogOfWar) {
			...
			rk.addHeaderItem("Fog of War Vision Range", Integer.toString(miscOptions.fogOfWarVisionRange.minValue)
					+ " ~ " + Integer.toString(miscOptions.fogOfWarVisionRange.maxValue));
		} else {
