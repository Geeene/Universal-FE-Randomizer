...
public class FE9ClassRandomizer {
	private static final DebugPrinter LOGGER = DebugPrinter.forKey(DebugPrinter.Key.FE9_RANDOM_CLASSES);
	public static final int rngSalt = 2744;

	public static void randomizePlayableCharacters(boolean includeLords, boolean includeThieves, boolean includeSpecial,
			boolean forceDifferent, boolean mixRaces, boolean crossGenders, boolean assignEvenly,
			FE9CharacterDataLoader charData, FE9ClassDataLoader classData, FE9ChapterDataLoader chapterData,
		...
		for (FE9Character character : charData.allPlayableCharacters()) {
			if (!charData.isModifiableCharacter(character)) {
				continue;
			}
			...
			if (!includeThieves && classData.isThiefClass(originalClass)) {
				continue;
			}
			if (!includeSpecial && classData.isSpecialClass(originalClass)) {
				continue;
			}
			...
			if (targetJID != null) {
				...
				List<FE9Class> possibleReplacements = possibleReplacementsForClass(originalClass, includeLords,
						includeThieves, includeSpecial, forceDifferent, mixRaces, crossGenders, true, classData);
				if (heronAssigned) {
				...
				if (pid.equals(FE9Data.Character.KIERAN.getPID()) || pid.equals(FE9Data.Character.BROM.getPID())
						|| pid.equals(FE9Data.Character.NEPHENEE.getPID())) {
					// Kieran, Brom, and Nephenee start out in locked cells. Them being a thief will
					// make their AI open doors automatically as Green units.
					possibleReplacements.removeIf(fe9Class -> {
						return classData.isThiefClass(fe9Class);
					});
				}

				LOGGER.log("Possible Classes: ");
				for (FE9Class charClass : possibleReplacements) {
					LOGGER.log("\t" + classData.getJIDForClass(charClass));
				}
				...
				LOGGER.log("Randomized " + pid + " from " + originalJID + " to " + targetJID);
			}

			if (classData.isLaguzClass(newClass)) {
				int startingGauge = rng.nextInt(20);
				LOGGER.log("Set Laguz starting gauge to " + startingGauge);
				...
				if (classData.isLaguzClass(originalClass) && charData.getUnpromotedAIDForCharacter(character) != null) {
					LOGGER.log("Set unpromoted AID to " + classData.getUnpromotedAIDForClass(newClass));
					charData.setUnpromotedAIDForCharacter(character, classData.getUnpromotedAIDForClass(newClass));
				}
				LOGGER.log("Set promoted AID to " + classData.getPromotedAIDForClass(newClass));
			...
			LOGGER.log("Old Weapon Levels: " + originalWeaponLevels);
			String classWeaponLevels = classData.getWeaponLevelsForClass(newClass);
			LOGGER.log("Target Class Weapon Levels: " + classWeaponLevels);
			boolean canUseKnives = (classData.getSID1ForClass(newClass) != null
					&& classData.getSID1ForClass(newClass).equals(FE9Data.Skill.EQUIP_KNIFE.getSID()))
					|| (classData.getSID2ForClass(newClass) != null
							&& classData.getSID2ForClass(newClass).equals(FE9Data.Skill.EQUIP_KNIFE.getSID()))
					|| (classData.getSID3ForClass(newClass) != null
							&& classData.getSID3ForClass(newClass).equals(FE9Data.Skill.EQUIP_KNIFE.getSID()));
			...
			int weaponTypesRemaining = classWeaponLevels.replace("-", "").replace("*", "").length()
					+ (classWeaponLevels.contains("*") ? 1 : 0);

			StringBuilder originalBuilder = new StringBuilder();
			if (originalLevels.contains("E")) {
				originalBuilder.append('E');
			}
			if (originalLevels.contains("D")) {
				originalBuilder.append('D');
			}
			if (originalLevels.contains("C")) {
				originalBuilder.append('C');
			}
			if (originalLevels.contains("B")) {
				originalBuilder.append('B');
			}
			if (originalLevels.contains("A")) {
				originalBuilder.append('A');
			}
			if (originalLevels.contains("S")) {
			...
			LOGGER.log("Weapon Level Set: " + originalLevels);
			...
			for (int i = 0; i < originalWeaponLevels.length(); i++) {
				...
				if (newClassChar == '-') {
					...
					if (starFulfilled) {
						...
						levels.append(rank);
						if (originalLevels.length() > 1) {
							originalLevels = originalLevels.replace("" + rank, "");
						}
						starFulfilled = true;
						if (i == 0) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_SWORD.getSID());
						}
						if (i == 1) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_LANCE.getSID());
						}
						if (i == 2) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_AXE.getSID());
						}
						if (i == 3) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_BOW.getSID());
						}
						if (i == 7) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_STAFF.getSID());
						}
						if (i == 8) {
							addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_KNIFE.getSID());
						}
					}
				} else {
			...
			LOGGER.log("Final Weapon Levels: " + finalWeaponLevelString);
			...
			if (skillsToRemove != null && !skillsToRemove.isEmpty()) {
				for (FE9Skill skill : skillsToRemove) {
					if (charData.getSID1ForCharacter(character) != null
							&& charData.getSID1ForCharacter(character).equals(skillData.getSID(skill))) {
						LOGGER.log("Removed " + charData.getSID1ForCharacter(character) + " from " + pid);
						charData.setSID1ForCharacter(character, null);
					} else if (charData.getSID2ForCharacter(character) != null
							&& charData.getSID2ForCharacter(character).equals(skillData.getSID(skill))) {
						LOGGER.log("Removed " + charData.getSID2ForCharacter(character) + " from " + pid);
						charData.setSID2ForCharacter(character, null);
					} else if (charData.getSID3ForCharacter(character) != null
							&& charData.getSID3ForCharacter(character).equals(skillData.getSID(skill))) {
						LOGGER.log("Removed " + charData.getSID3ForCharacter(character) + " from " + pid);
			...
			if (addedWeaponDiscipline != null) {
				necessarySkills.add(0, addedWeaponDiscipline);
			}
			if (necessarySkills != null && !necessarySkills.isEmpty()) {
				for (FE9Skill skill : necessarySkills) {
					if (charData.getSID1ForCharacter(character) == null) {
						...
						LOGGER.log("Added " + charData.getSID1ForCharacter(character) + " to " + pid);
					} else if (charData.getSID2ForCharacter(character) == null) {
						charData.setSID2ForCharacter(character, skillData.getSID(skill));
						LOGGER.log("Added " + charData.getSID2ForCharacter(character) + " to " + pid);
					} else if (charData.getSID3ForCharacter(character) == null) {
						charData.setSID3ForCharacter(character, skillData.getSID(skill));
						LOGGER.log("Added " + charData.getSID3ForCharacter(character) + " to " + pid);
					} else {
						break;
					}
				}
			...
			if (matchingOccultSkill != null) {
				...
				if (skillData.isOccultSkill(skill1)) {
					charData.setSID1ForCharacter(character, skillData.getSID(matchingOccultSkill));
				}
				if (skillData.isOccultSkill(skill2)) {
					charData.setSID2ForCharacter(character, skillData.getSID(matchingOccultSkill));
				}
				if (skillData.isOccultSkill(skill3)) {
					charData.setSID3ForCharacter(character, skillData.getSID(matchingOccultSkill));
				}
			}

			// Update chapter data (class, weapons, and equipment)
			for (FE9Data.Chapter chapter : FE9Data.Chapter.values()) {
				LOGGER.log("Processing Chapter: " + chapter.toString());
					...
					for (String unitID : army.getAllUnitIDs()) {
						...
						if (army.getPIDForUnit(unit).equals(pid)) {
							...
							if (itemData.laguzWeaponForJID(targetJID) != null) {
								...
								if (classData.isLaguzClass(originalClass) && !classData.isLaguzClass(newClass)) {
									weaponCount++; // Former Laguz units will not have any weapons, so we should give
													// them one.
								}

								List<WeaponRank> equippedRanks = new ArrayList<WeaponRank>();
								if (iid1 != null) {
									equippedRanks.add(itemData.weaponRankForItem(itemData.itemWithIID(iid1)));
								}
								if (iid2 != null) {
									equippedRanks.add(itemData.weaponRankForItem(itemData.itemWithIID(iid2)));
								}
								if (iid3 != null) {
									equippedRanks.add(itemData.weaponRankForItem(itemData.itemWithIID(iid3)));
								}
								if (iid4 != null) {
									equippedRanks.add(itemData.weaponRankForItem(itemData.itemWithIID(iid4)));
								}
								equippedRanks.removeIf(rank -> (rank == WeaponRank.UNKNOWN || rank == WeaponRank.NONE));

								Map<WeaponType, WeaponRank> weaponLevelsMap = itemData
										.weaponLevelsForWeaponString(finalWeaponLevelString);
								if (!classData.canClassUseLightMagic(newClass)) { // Light magic keys off of staff
																					// ranks, but not every staff user
																					// uses light magic.
									weaponLevelsMap.remove(WeaponType.LIGHT);
								}
								List<WeaponType> types = weaponLevelsMap.keySet().stream()
										.sorted(WeaponType.getComparator()).collect(Collectors.toList());
								// For whatever reason, Assassin doesn't have any weapon levels defined or the
								// EQUIPKNIFE skill.
								if (canUseKnives || targetJID.equals(FE9Data.CharacterClass.ASSASSIN.getJID())) {
									types.add(WeaponType.KNIFE);
									weaponLevelsMap.put(WeaponType.KNIFE,
											itemData.highestRankInString(finalWeaponLevelString));
								}

								LOGGER.log(weaponCount + " weapons to assign.");
								for (WeaponType type : types) {
									LOGGER.log("Type: " + type.toString() + " (" + weaponLevelsMap.get(type) + ")");
								}
								if (!types.isEmpty()) {
									for (int i = 0; i < weaponCount; i++) {
										WeaponType randomUsableType = types.get(rng.nextInt(types.size()));
										LOGGER.log("Selected type: " + randomUsableType.toString());
										WeaponRank usableRank = equippedRanks.size() > i ? equippedRanks.get(i)
												: weaponLevelsMap.get(randomUsableType);
										List<FE9Item> replacements = itemData.weaponsOfRankAndType(usableRank,
												randomUsableType);
										if (replacements.isEmpty()) {
											WeaponRank adjacentRank = usableRank.lowerRank();
											if (adjacentRank == WeaponRank.NONE && usableRank.higherRank()
													.isLowerThan(weaponLevelsMap.get(randomUsableType))) {
												adjacentRank = usableRank.higherRank();
											}
											replacements = itemData.weaponsOfRankAndType(adjacentRank,
													randomUsableType);
										}
										...
										if (specialWeapons != null) {
											replacements.addAll(specialWeapons.stream().filter(item -> {
												boolean isCorrectRank = !itemData.weaponRankForItem(item)
														.isHigherThan(usableRank);
												boolean isCorrectType = itemData
														.weaponTypeForItem(item) == randomUsableType;
												if (isCorrectRank && isCorrectType) {
													LOGGER.log("Added weapon " + itemData.iidOfItem(item)
															+ " as special class weapon.");
													return true;
												}
												return false;
											}).collect(Collectors.toList()));
										...
										LOGGER.log("Possible replacements: ");
										for (FE9Item weapon : replacements) {
											LOGGER.log("\t" + itemData.iidOfItem(weapon));
										}

										if (!replacements.isEmpty()) {
											FE9Item weapon = replacements.get(rng.nextInt(replacements.size()));
											LOGGER.log("Selected: " + itemData.iidOfItem(weapon));
											weapons.add(weapon);
										}
									}
								}
							...
							LOGGER.log("Assigned Weapons: ");
							weapons.forEach(weapon -> {
								LOGGER.log(itemData.iidOfItem(weapon));
							});

							army.setWeapon1ForUnit(unit,
									weapons.size() > 0 ? itemData.iidOfItem(weapons.get(0)) : null);
							army.setWeapon2ForUnit(unit,
									weapons.size() > 1 ? itemData.iidOfItem(weapons.get(1)) : null);
							army.setWeapon3ForUnit(unit,
									weapons.size() > 2 ? itemData.iidOfItem(weapons.get(2)) : null);
							army.setWeapon4ForUnit(unit,
									weapons.size() > 3 ? itemData.iidOfItem(weapons.get(3)) : null);
							...
							if (item1 != null && itemData.getImportantEquipment().contains(item1)) {
								equipment.add(item1);
							}
							if (item2 != null && itemData.getImportantEquipment().contains(item2)) {
								equipment.add(item2);
							}
							if (item3 != null && itemData.getImportantEquipment().contains(item3)) {
								equipment.add(item3);
							}
							if (item4 != null && itemData.getImportantEquipment().contains(item4)) {
								equipment.add(item4);
							}
							...
							LOGGER.log("Assigned Equipment: ");
							equipment.forEach(equip -> {
								LOGGER.log(itemData.iidOfItem(equip));
							});

							army.setItem1ForUnit(unit,
									equipment.size() > 0 ? itemData.iidOfItem(equipment.get(0)) : null);
							army.setItem2ForUnit(unit,
									equipment.size() > 1 ? itemData.iidOfItem(equipment.get(1)) : null);
							army.setItem3ForUnit(unit,
									equipment.size() > 2 ? itemData.iidOfItem(equipment.get(2)) : null);
							army.setItem4ForUnit(unit,
									equipment.size() > 3 ? itemData.iidOfItem(equipment.get(3)) : null);
	...
	public static void randomizeBossCharacters(boolean forceDifferent, boolean mixRaces, boolean crossGenders,
			FE9CharacterDataLoader charData, FE9ClassDataLoader classData, FE9ChapterDataLoader chapterData,
			FE9SkillDataLoader skillData, FE9ItemDataLoader itemData, Random rng) {
		...
		for (FE9Character character : charData.allBossCharacters()) {
			...
			if (fe9Char != null && fe9Char.isUnsafeForRandomization()) {
				continue;
			}
			String originalJID = charData.getJIDForCharacter(character);
			FE9Class originalClass = classData.classWithID(originalJID);
			...
			LOGGER.log("Randomizing Boss " + pid);
						...
						for (String unitID : army.getAllUnitIDs()) {
							...
							if (army.getPIDForUnit(unit).equals(pid)) {
								...
								if (!armyJID.equals(originalJID)) {
									LOGGER.log("JID mismatch from character data (" + originalJID + ") and army data ("
											+ armyJID + "). Defaulting to " + armyJID);
									originalJID = armyJID;
					...
					for (String unitID : army.getAllUnitIDs()) {
						...
						if (army.getPIDForUnit(unit).equals(pid)) {
							...
							if (itemData.laguzWeaponForJID(targetJID) != null) {
								...
								if (army.getSkill1ForUnit(unit) == null) {
									army.setSkill1ForUnit(unit, FE9Data.Skill.FERAL.getSID());
								} else if (army.getSkill2ForUnit(unit) == null) {
									army.setSkill2ForUnit(unit, FE9Data.Skill.FERAL.getSID());
								} else if (army.getSkill3ForUnit(unit) == null) {
									army.setSkill3ForUnit(unit, FE9Data.Skill.FERAL.getSID());
								}
							} else {
								int weaponCount = 0;
								String iid1 = army.getWeapon1ForUnit(unit);
...
//									if (specialWeapons != null) { replacements.addAll(specialWeapons); }
									...
									if (!replacements.isEmpty()) {
										...
										LOGGER.log("Selected: " + itemData.iidOfItem(weapon));
										weapons.add(weapon);
										if (itemData.isSiegeTome(weapon)) {
											hasSiegeTome = true;
										} else {
											hasNonSiegeTome = true;
										}

										if (itemData.isRanged(weapon)) {
											hasRangedOption = true;
										}
										if (itemData.isMelee(weapon)) {
											hasMeleeOption = true;
										}
									}
								}
								...
								if (hasSiegeTome && !hasNonSiegeTome) {
									...
									while (types.size() > 0) {
										...
										List<FE9Item> replacements = itemData.weaponsOfRankAndType(usableRank,
												usableType);
										replacements.removeIf(weapon -> {
											return itemData.isSiegeTome(weapon);
										});
										while (replacements.isEmpty()) {
											usableRank = usableRank.lowerRank();
											if (usableRank == WeaponRank.NONE) {
												break;
											}
											replacements = itemData.weaponsOfRankAndType(usableRank.lowerRank(),
													usableType);
											replacements.removeIf(weapon -> {
												return itemData.isSiegeTome(weapon);
											});
										...
										if (!replacements.isEmpty()) {
											FE9Item additionalWeapon = replacements
													.get(rng.nextInt(replacements.size()));
											LOGGER.log("Additional Weapon: " + itemData.iidOfItem(additionalWeapon));
								...
								if (hasRangedOption && !hasMeleeOption) {
									...
									while (types.size() > 0) {
										...
										List<FE9Item> replacements = itemData.weaponsOfRankAndType(usableRank,
												usableType);
										replacements.removeIf(weapon -> {
											return !itemData.isMelee(weapon);
										});
										while (replacements.isEmpty()) {
											usableRank = usableRank.lowerRank();
											if (usableRank == WeaponRank.NONE) {
								...
								if (hasMeleeOption && !hasRangedOption) {
									...
									while (types.size() > 0) {
										...
										List<FE9Item> replacements = itemData.weaponsOfRankAndType(usableRank,
												usableType);
										replacements.removeIf(weapon -> {
											return !itemData.isRanged(weapon);
										});
										while (replacements.isEmpty()) {
											usableRank = usableRank.lowerRank();
											if (usableRank == WeaponRank.NONE) {
							...
							if (unit.willDropItem1()) {
								existingDrops.add(itemData.itemWithIID(army.getItem1ForUnit(unit)));
							}
							if (unit.willDropItem2()) {
								existingDrops.add(itemData.itemWithIID(army.getItem2ForUnit(unit)));
							}
							if (unit.willDropItem3()) {
								existingDrops.add(itemData.itemWithIID(army.getItem3ForUnit(unit)));
							}
							if (unit.willDropItem4()) {
								existingDrops.add(itemData.itemWithIID(army.getItem4ForUnit(unit)));
							}
							...
							if (equipment.size() > 0 && existingDrops.contains(equipment.get(0))) {
								unit.setWillDropItem1(true);
							}
							if (equipment.size() > 1 && existingDrops.contains(equipment.get(1))) {
								unit.setWillDropItem2(true);
							}
							if (equipment.size() > 2 && existingDrops.contains(equipment.get(2))) {
								unit.setWillDropItem3(true);
							}
							if (equipment.size() > 3 && existingDrops.contains(equipment.get(3))) {
	...
	public static void randomizeMinionCharacters(int chance, boolean forceDifferent, boolean mixRaces,
			boolean crossGenders, FE9CharacterDataLoader charData, FE9ClassDataLoader classData,
			FE9ChapterDataLoader chapterData, FE9SkillDataLoader skillData, FE9ItemDataLoader itemData, Random rng) {
		...
		for (FE9Data.Chapter chapter : FE9Data.Chapter.allChapters()) {
			for (FE9ChapterArmy army : chapterData.armiesForChapter(chapter)) {
				for (String unitID : army.getAllUnitIDs()) {
					...
					if (charData.isRestrictedMinionCharacterPID(pid)) {
						continue;
					}

					String originalJID = army.getJIDForUnit(unit);
					FE9Class originalClass = classData.classWithID(originalJID);
					if (classData.isPacifistClass(originalClass)) {
						continue;
					} // Don't change healers.

					if (chapter.hasWaterSpawningBandits() && (originalJID.equals(FE9Data.CharacterClass.BANDIT.getJID())
					...
					if (isDaeinMinion) {
						replacementClasses.removeIf(charClass -> {
							return !charData.availableJIDsForDaeinMinions()
									.contains(classData.getJIDForClass(charClass));
						});
					} else if (isPID14Minion) {
						replacementClasses.removeIf(charClass -> {
							return !charData.availableJIDsForPID14Minions()
									.contains(classData.getJIDForClass(charClass));
						});
					}
					if (replacementClasses.isEmpty()) {
					...
					if (isDaeinMinion) {
						// Daein minions have classes built into them, so we have to replace PIDs if we
						// change their class too.
						...
						if (replacementCharacters == null || replacementCharacters.isEmpty()) {
							// Try ZAKO
							LOGGER.error("Replacing Daein minion with ZAKO.");
							army.setPIDForUnit(unit, "PID_ZAKO");
						} else {
							FE9Character replacement = replacementCharacters
									.get(rng.nextInt(replacementCharacters.size()));
						...
						if (replacementCharacters == null || replacementCharacters.isEmpty()) {
							// This shouldn't happen with the new logic, but try ZAKO anyway if we get here
							// somehow.
							LOGGER.error("Replacing PID_14 minion with ZAKO.");
							army.setPIDForUnit(unit, "PID_ZAKO");
						} else {
							FE9Character replacement = replacementCharacters
									.get(rng.nextInt(replacementCharacters.size()));
					...
					for (int i = 0; i < classWeaponLevels.length(); i++) {
						...
						if (newClassChar == '-') {
							...
							if (starFulfilled) {
								...
								starFulfilled = true;
								if (i == 0) {
									addedWeaponDiscipline = skillData
											.getSkillWithSID(FE9Data.Skill.EQUIP_SWORD.getSID());
								}
								if (i == 1) {
									addedWeaponDiscipline = skillData
											.getSkillWithSID(FE9Data.Skill.EQUIP_LANCE.getSID());
								}
								if (i == 2) {
									addedWeaponDiscipline = skillData.getSkillWithSID(FE9Data.Skill.EQUIP_AXE.getSID());
								}
								...
								if (i == 7) {
									addedWeaponDiscipline = skillData
											.getSkillWithSID(FE9Data.Skill.EQUIP_STAFF.getSID());
								}
								if (i == 8) {
									addedWeaponDiscipline = skillData
											.getSkillWithSID(FE9Data.Skill.EQUIP_KNIFE.getSID());
								}
							}
						} else {
							if (weaponTypesRemaining > 1) {
					...
					if (itemData.laguzWeaponForJID(targetJID) != null) {
						...
						Map<WeaponType, WeaponRank> weaponLevelsMap = itemData
								.weaponLevelsForWeaponString(finalWeaponLevelString);
						if (!classData.canClassUseLightMagic(newClass)) { // Light magic keys off of staff ranks, but
																			// not every staff user uses light magic.
							weaponLevelsMap.remove(WeaponType.LIGHT);
						}
						List<WeaponType> types = weaponLevelsMap.keySet().stream().sorted(WeaponType.getComparator())
								.collect(Collectors.toList());
						// For whatever reason, Assassin doesn't have any weapon levels defined or the
						// EQUIPKNIFE skill.
						if (canUseKnives || targetJID.equals(FE9Data.CharacterClass.ASSASSIN.getJID())) {
						...
						for (int i = 0; i < weaponCount; i++) {
							...
							if (replacements.isEmpty()) {
								WeaponRank adjacentRank = usableRank.lowerRank();
								if (adjacentRank == WeaponRank.NONE
										&& usableRank.higherRank().isLowerThan(weaponLevelsMap.get(randomUsableType))) {
									adjacentRank = usableRank.higherRank();
								}
								replacements = itemData.weaponsOfRankAndType(adjacentRank, randomUsableType);
							}
...
//							if (specialWeapons != null) { replacements.addAll(specialWeapons); }
	...
	private static List<FE9Class> possibleReplacementsForClass(FE9Class originalClass, boolean includeLords,
			boolean includeThieves, boolean includeSpecial, boolean forceDifferent, boolean mixRaces,
			boolean crossGenders, boolean forPlayerCharacter, FE9ClassDataLoader classData) {
