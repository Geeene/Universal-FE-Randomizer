...
public class FE4SkillsRandomizer {
	private static final DebugPrinter LOGGER = DebugPrinter.forKey(DebugPrinter.Key.FE4_SKILL_RANDOM);
	...
	public static void shufflePlayableCharacterSkills(SkillsOptions options, CharacterDataLoader charData, Random rng) {
		if (options.separatePoolsByGeneration) {
			// Collect all of the skills from all characters. Go ahead and wipe them at the
			// same time so we don't have to iterate it again.
			...
			for (FE4StaticCharacter staticChar : gen1Characters) {
				...
				for (FE4Data.SkillSlot1 slot1 : slot1Skills) {
					universalPool.addItem(slot1.generalSkill());
					characterPool.addItem(fe4Char);
				}
				for (FE4Data.SkillSlot2 slot2 : slot2Skills) {
					universalPool.addItem(slot2.generalSkill());
					characterPool.addItem(fe4Char);
				}
				for (FE4Data.SkillSlot3 slot3 : slot3Skills) {
					universalPool.addItem(slot3.generalSkill());
					characterPool.addItem(fe4Char);
				}
			...
			for (FE4Data.Character fe4Char : assignedSkills.keySet()) {
				...
				for (FE4Data.Character linked : fe4Char.linkedCharacters()) {
					...
					if (staticChar == null) {
						staticChar = charData.getStaticCharacter(linked);
					}
					if (staticChar == null) {
						continue;
					}
	...
	public static void randomizePlayableCharacterSkills(SkillsOptions options, CharacterDataLoader charData,
			Random rng) {

		WeightedDistributor<Integer> skillCountDistributor = skillCountDistributionFromOptions(options);

		LOGGER.log("Skill Counts:");
		LOGGER.log(String.format("Zero Skills: %.2f%%", skillCountDistributor.chanceOfResult(0) * 100));
		LOGGER.log(String.format("One Skill: %.2f%%", skillCountDistributor.chanceOfResult(1) * 100));
		LOGGER.log(String.format("Two Skills: %.2f%%", skillCountDistributor.chanceOfResult(2) * 100));
		LOGGER.log(String.format("Three Skills: %.2f%%", skillCountDistributor.chanceOfResult(3) * 100));

		WeightedDistributor<FE4Data.Skill> skillDistributor = skillDistributionFromOptions(options);

		LOGGER.log("Skill Weights:");
		for (FE4Data.Skill skill : FE4Data.Skill.values()) {
			LOGGER.log(String.format("%s: %.2f%%", skill.toString(), skillDistributor.chanceOfResult(skill) * 100));
		}
		...
		for (FE4StaticCharacter staticChar : gen1Characters) {
			...
			if (skillsAssigned != null) {
				...
				skillsAssigned = assignSkillsToStaticCharacter(options, staticChar, fe4Char, numberOfSkills,
						skillDistributor, rng);
	...
	public static WeightedDistributor<Integer> skillCountDistributionFromOptions(SkillsOptions options) {
		...
		if (options.skillCounts.zeroSkillsChance.enabled) {
			skillCountDistributor.addItem(0,
					options.skillCounts.zeroSkillsChance.weight.integerWeightUsingLinearWeight(1, 0));
		}
		if (options.skillCounts.oneSkillChance.enabled) {
			skillCountDistributor.addItem(1,
					options.skillCounts.oneSkillChance.weight.integerWeightUsingLinearWeight(1, 0));
		}
		if (options.skillCounts.twoSkillChance.enabled) {
			skillCountDistributor.addItem(2,
					options.skillCounts.twoSkillChance.weight.integerWeightUsingLinearWeight(1, 0));
		}
		if (options.skillCounts.threeSkillChance.enabled) {
			skillCountDistributor.addItem(3,
					options.skillCounts.threeSkillChance.weight.integerWeightUsingLinearWeight(1, 0));
		}
	...
	public static WeightedDistributor<FE4Data.Skill> skillDistributionFromOptions(SkillsOptions options) {
		...
		if (options.skillWeights.adeptWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.ADEPT,
					options.skillWeights.adeptWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.astraWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.ASTRA,
					options.skillWeights.astraWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.bargainWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.BARGAIN, options.skillWeights.bargainWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.chargeWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.CHARGE,
					options.skillWeights.chargeWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.charmWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.CHARM,
					options.skillWeights.charmWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.criticalWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.CRITICAL, options.skillWeights.criticalWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.lunaWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.LUNA,
					options.skillWeights.lunaWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.miracleWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.MIRACLE, options.skillWeights.miracleWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.nihilWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.NIHIL,
					options.skillWeights.nihilWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.paragonWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.PARAGON, options.skillWeights.paragonWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.renewalWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.RENEWAL, options.skillWeights.renewalWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.solWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.SOL,
					options.skillWeights.solWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.vantageWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.VANTAGE, options.skillWeights.vantageWeight.weight
					.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}
		if (options.skillWeights.wrathWeight.enabled) {
			skillDistributor.addItem(FE4Data.Skill.WRATH,
					options.skillWeights.wrathWeight.weight.integerWeightUsingLinearWeight(linearSlope, linearOffset));
		}

		return skillDistributor;
	}

	private static List<FE4Data.Skill> assignSkillsToStaticCharacter(SkillsOptions options,
			FE4StaticCharacter staticChar, FE4Data.Character fe4Char, int numberOfSkills,
			WeightedDistributor<FE4Data.Skill> skillDistributor, Random rng) {
		staticChar.setSkillSlot1Value(0);
		staticChar.setSkillSlot2Value(0);
		staticChar.setSkillSlot3Value(0);

		if (numberOfSkills == 0) {
			return new ArrayList<FE4Data.Skill>();
		}
		...
		for (int i = 0; i < numberOfSkills; i++) {
			...
			if (workingSkillDistributor.possibleResults().isEmpty()) {
				break;
			}
		}
