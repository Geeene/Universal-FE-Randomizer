...
public class TerrainView extends YuneView<TerrainOptions> {
    ...
    private Map<TerrainTable.TerrainTableType, Button> enabledMap;
    private Map<TerrainTable.TerrainTableType, MinMaxControl> rangeMap;
    private Map<TerrainTable.TerrainTableType, Spinner> spinnerMap;
    private Map<TerrainTable.TerrainTableType, Label> spinnerLabelMap;


    public TerrainView(Composite parent) {
        super(parent);
    }
    ...
    protected String getGroupTooltip() {
        return "Randomization of Terrain Bonuses";
    }

    @Override
    protected void compose() {
        enabledMap = new HashMap<>();
        spinnerLabelMap = new HashMap<>();
        ...
        keepSafeTiles = new Button(group, SWT.CHECK);
        ...
        for (TerrainTable.TerrainTableType terrainType : Arrays.asList(AVOID, DEF, RES, HEALING, STATUS_RECOVERY)) {
            Composite subGroup = createCompositeForTerrainType(group, terrainType);

            formData = new FormData();
            formData.left = new FormAttachment(group, 10);
            formData.top = new FormAttachment(previousControl, 5);
            formData.right = new FormAttachment(100, -5);
            subGroup.setLayoutData(formData);
            previousControl = subGroup;
        }
    ...
    @Override
    public void initialize(TerrainOptions options) {
        if (options == null) {
