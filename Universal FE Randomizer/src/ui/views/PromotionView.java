...
public class PromotionView extends YuneView<PromotionOptions> {
    ...
    public PromotionView(Composite parent, GameType type) {
        super(parent, type);
    }
    ...
    protected void compose() {
        container = new Group(this.group, SWT.NONE);
        ...
        allowEnemyClassButton.setToolTipText(getEnemyClassTooltip());
    ...
    private String getEnemyClassTooltip() {
        String s = null;
        if (GameType.FE8.equals(type)) {
            s = "Allows units to promote into Special not usually usable classes (Necromancer, Fleet).";
        } else if (GameType.FE7.equals(type)) {
            s = "Allows units to promote into Special not usually usable classes (Archsage, Dark Druid).";
        } else if (GameType.FE6.equals(type)) {
            s = "Allows units to promote into Special not usually usable classes (King).";
        } else if (GameType.FE4.equals(type)) {
            s = "Allows units to promote into enemy-only classes like Baron, Queen, and Emperor.";
        }
        return s;
    }

    @Override
    public void initialize(PromotionOptions options) {
        if (options == null) {
            // Shouldn't happen.
            return;
        }
